...
char *ctcp_encode ( const char *msg, int msg_size )
{
	...
	for ( offset = 0, out_size = 0; offset <= msg_size; offset++, out_size++ ) {
		...
		if ( out_size >= out_capacity - 1 ) {
			out_capacity += msg_size/2 ;
			out_msg = (char *)realloc(out_msg, out_capacity*sizeof(char));
		}
...
ctcp_extended_data_list *ctcp_get_extended_data ( const char *in_msg, int size)
{
	...
	while ( (msg - msg_start) < size && (tag_loc = strchr(msg, CTCP_TAG)) != NULL ) {
		
		*tag_loc = '\0' ;

		if (data_list == NULL) {
			data_list = (ctcp_extended_data_list *)calloc(1, sizeof(ctcp_extended_data_list));
			list_head = data_list ;
		}
		else {
			data_list -> next = (ctcp_extended_data_list *)calloc(1, sizeof(ctcp_extended_data_list));
			data_list = data_list -> next ;
		}
...
char *ctcp_gen_extended_data_request ( int type, const char *msg )
{
	...
	out_msg_len = ctcp_command [ type ].length + 2 + (msg?strlen(msg)+1:0) ;
...
char *ctcp_gen_version_response ( char *client_name, char *client_version, char *client_env )
{
	char *out_msg = NULL ;
...
ctcp_version *ctcp_got_version (const char *msg)
{
	...
	if ( version_offset ) {
		ctcp_strndup(out -> version, (name_offset+1), (version_offset - name_offset - 1));
...
char *ctcp_gen_source_response ( char *source_host, char *source_path, char *source_file )
{
	char *out_msg = NULL ;
...
char *ctcp_gen_time_response (void)
{
	...
	out_msg_len = ctcp_command [ CTCP_TIME ].length + 4 + (cur_time?strlen(cur_time):0) ;
	...
	for(count = CTCP_NONE + 1; count < CTCP_DATA_COUNT; count++) {
		if (!strcmp(command, ctcp_command [count].data)) {
			...
			out_msg = (char *)calloc( (ctcp_command [count].length + 
						   strlen(ctcp_command [count].clientinfo_description) + 1 )
						 , sizeof(char) );

			strcpy(out_msg, ctcp_command [count].data);
			strcat(out_msg, ctcp_command [count].clientinfo_description);
...
char *ctcp_gen_ping_response(char *timestamp)
{
	...
	out_msg_len = ctcp_command [ CTCP_PING ].length + 3 + (timestamp?strlen(timestamp):0) ;
