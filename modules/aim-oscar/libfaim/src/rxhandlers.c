	...
	int (*modfirst) (aim_session_t *, aim_module_t *))
{
	...
	faimdprintf(sess, 1,
		"registered module %s (family 0x%04x, version = 0x%04x, tool 0x%04x, tool version 0x%04x)\n",
		mod->name, mod->family, mod->version, mod->toolid,
		mod->toolversion);
...
static int consumenonsnac(aim_session_t *sess, aim_frame_t *rx, fu16_t family,
	fu16_t subtype)
{
	aim_module_t *cur;
	aim_modsnac_t snac;
...
static int negchan_middle(aim_session_t *sess, aim_frame_t *fr)
{
	...
	if ((userfunc = aim_callhandler(sess, fr->conn, AIM_CB_FAM_SPECIAL,
...
faim_internal int bleck(aim_session_t *sess, aim_frame_t *frame, ...)
{
	...
	if (frame->hdr.flap.type == 0x02) {
		...
		if ((family < maxf) && (subtype + 1 < maxs)
			&& (literals[family][subtype] != NULL))
			faimdprintf(sess, 0,
				"bleck: channel %s: null handler for %04x/%04x (%s)\n",
				channels[frame->hdr.flap.type], family, subtype,
				literals[family][subtype + 1]);
		else
			faimdprintf(sess, 0,
				"bleck: channel %s: null handler for %04x/%04x (no literal)\n",
				channels[frame->hdr.flap.type], family,
				subtype);
	} else {

		if (frame->hdr.flap.type <= maxchannels)
			faimdprintf(sess, 0, "bleck: channel %s (0x%02x)\n",
				channels[frame->hdr.flap.type],
				frame->hdr.flap.type);
		else
...
faim_export int aim_conn_addhandler(aim_session_t *sess, aim_conn_t *conn,
	fu16_t family, fu16_t type, aim_rxcallback_t newhandler, fu16_t flags)
{
	...
	if (!(newcb = (struct aim_rxcblist_s *)calloc(1,
				sizeof(struct aim_rxcblist_s))))
		return -1;
	...
	if (!conn->handlerlist)
		...
		for (cur = (struct aim_rxcblist_s *)conn->handlerlist;
	...
	aim_conn_t *conn, fu16_t family, fu16_t type)
{
	...
	if (type == AIM_CB_SPECIAL_DEFAULT) {
		faimdprintf(sess, 1,
			"aim_callhandler: no default handler for family 0x%04x\n",
			family);
	...
	aim_conn_t *src)
{
	...
	for (cur = (struct aim_rxcblist_s *)src->handlerlist; cur;
		cur = cur->next) {
	...
	for (cur = sess->queue_incoming, i = 0; cur; cur = cur->next, i++) {
		...
		if (cur->hdrtype == AIM_FRAMETYPE_FLAP) {
			...
			if (cur->conn->type == AIM_CONN_TYPE_RENDEZVOUS) {
				...
				/* not possible */
				faimdprintf(sess, 0,
					"rxdispatch called on LISTENER connection!\n");
				cur->handled = 1;
				continue;
			}
		}
