 ...
 */
static struct aim_ssi_item *aim_ssi_itemlist_rebuildgroup(struct aim_ssi_item
	*list, const char *name)
{
	...
	/* Find the group */
	if (!(group = aim_ssi_itemlist_finditem(list, name, NULL,
				AIM_SSI_TYPE_GROUP)))
		return NULL;
	...
	if (group->gid == 0x0000) {
		...
		for (cur = list; cur; cur = cur->next)
			if ((cur->gid == group->gid)
				&& (cur->type == AIM_SSI_TYPE_BUDDY))
				newlen += 2;
	}
 ...
 */
static struct aim_ssi_item *aim_ssi_itemlist_add(struct aim_ssi_item **list,
	const char *name, fu16_t gid, fu16_t bid, fu16_t type,
	aim_tlvlist_t *data)
{
	int i;
	...
	if (*list) {
		if ((new->gid < (*list)->gid) || ((new->gid == (*list)->gid)
				&& (new->bid < (*list)->bid))) {
			new->next = *list;
			*list = new;
		} else {
 ...
 */
static int aim_ssi_itemlist_del(struct aim_ssi_item **list,
	struct aim_ssi_item *del)
{
	...
	if (*list == del) {
		*list = (*list)->next;
		...
		for (cur = *list; (cur->next && (cur->next != del));
			cur = cur->next) ;
		if (cur->next)
 ...
 */
static int aim_ssi_itemlist_cmp(struct aim_ssi_item *cur1,
	struct aim_ssi_item *cur2)
{
	...
	if ((cur1->data && cur2->data)
		&& (aim_tlvlist_cmp(cur1->data, cur2->data)))
		return 4;
...
faim_export int aim_ssi_itemlist_valid(struct aim_ssi_item *list,
 ...
 */
faim_export struct aim_ssi_item *aim_ssi_itemlist_find(struct aim_ssi_item
	*list, fu16_t gid, fu16_t bid)
{
	struct aim_ssi_item *cur;
	for (cur = list; cur; cur = cur->next)
 ...
 * @return Return a pointer to the item if found, else return NULL;
 */
faim_export struct aim_ssi_item *aim_ssi_itemlist_finditem(struct aim_ssi_item
	*list, const char *gn, const char *sn, fu16_t type)
{
	struct aim_ssi_item *cur;
	if (!list)
		return NULL;
	...
	if (gn && sn) {		/* For finding buddies in groups */
		for (cur = list; cur; cur = cur->next)
			if ((cur->type == type) && (cur->name)
				&& !(aim_sncmp(cur->name, sn))) {
				struct aim_ssi_item *curg;
				for (curg = list; curg; curg = curg->next)
					if ((curg->type == AIM_SSI_TYPE_GROUP)
						&& (curg->gid == cur->gid)
						&& (curg->name)
						&& !(aim_sncmp(curg->name, gn)))
						return cur;
			}

	} else if (gn) {	/* For finding groups */
		for (cur = list; cur; cur = cur->next) {
			if ((cur->type == type) && (cur->bid == 0x0000)
				&& (cur->name) && !(aim_sncmp(cur->name, gn))) {
				return cur;
			}
		}
 ...
 */
faim_export struct aim_ssi_item *aim_ssi_itemlist_exists(struct aim_ssi_item
	*list, const char *sn)
{
	struct aim_ssi_item *cur;
	if (!list || !sn)
		return NULL;
	for (cur = list; cur; cur = cur->next)
		if ((cur->type == AIM_SSI_TYPE_BUDDY) && (cur->name)
			&& (!aim_sncmp(cur->name, sn)))
			return cur;
	return NULL;
}
 ...
 * @return Return a pointer to the name of the item if found, else return NULL;
 */
faim_export char *aim_ssi_itemlist_findparentname(struct aim_ssi_item *list,
	const char *sn)
{
...
{
	struct aim_ssi_item *cur =
		aim_ssi_itemlist_finditem(list, NULL, NULL,
		AIM_SSI_TYPE_PDINFO);
	if (cur) {
 ...
 *         this returned value!
 */
faim_export char *aim_ssi_getalias(struct aim_ssi_item *list, const char *gn,
	const char *sn)
{
	struct aim_ssi_item *cur =
		aim_ssi_itemlist_finditem(list, gn, sn, AIM_SSI_TYPE_BUDDY);
	if (cur) {
		aim_tlvlist_t *tlvlist = cur->data;
		if (tlvlist) {
...
static int aim_ssi_sync(aim_session_t *sess)
{
	...
	if (!sess->ssi.pending) {
		for (cur1 = sess->ssi.local; cur1; cur1 = cur1->next) {
			if (!aim_ssi_itemlist_find(sess->ssi.official,
					cur1->gid, cur1->bid)) {
				new = (struct aim_ssi_tmp *)malloc(sizeof(struct
						aim_ssi_tmp));
				...
				new->next = NULL;
				if (sess->ssi.pending) {
					for (cur = sess->ssi.pending; cur->next;
						cur = cur->next) ;
					cur->next = new;
				} else
	...
	if (!sess->ssi.pending) {
		for (cur1 = sess->ssi.official; cur1; cur1 = cur1->next) {
			if (!aim_ssi_itemlist_find(sess->ssi.local, cur1->gid,
					cur1->bid)) {
				new = (struct aim_ssi_tmp *)malloc(sizeof(struct
						aim_ssi_tmp));
	...
	if (!sess->ssi.pending) {
		for (cur1 = sess->ssi.local; cur1; cur1 = cur1->next) {
			cur2 = aim_ssi_itemlist_find(sess->ssi.official,
				cur1->gid, cur1->bid);
...
faim_export int aim_ssi_cleanlist(aim_session_t *sess)
{
	...
	while (cur) {
		...
		} else if ((cur->type == AIM_SSI_TYPE_BUDDY)
			&& ((cur->gid == 0x0000)
				|| (!aim_ssi_itemlist_find(sess->ssi.local,
						cur->gid, 0x0000)))) {
			aim_ssi_addbuddy(sess, cur->name, "orphans", NULL, NULL,
				NULL, 0);
	...
	if ((cur = aim_ssi_itemlist_find(sess->ssi.local, 0x0000, 0x0000))
		&& (!cur->data))
		aim_ssi_itemlist_del(&sess->ssi.local, cur);
	...
	const char *smsnum, int needauth)
{
	...
	/* Find the parent */
	if (!(parent = aim_ssi_itemlist_finditem(sess->ssi.local, group, NULL,
				AIM_SSI_TYPE_GROUP))) {
		/* Find the parent's parent (the master group) */
		if (!(parent = aim_ssi_itemlist_find(sess->ssi.local, 0x0000,
					0x0000)))
			if (!(parent = aim_ssi_itemlist_add(&sess->ssi.local,
						NULL, 0x0000, 0x0000,
						AIM_SSI_TYPE_GROUP, NULL)))
				return -ENOMEM;
		/* Add the parent */
		if (!(parent = aim_ssi_itemlist_add(&sess->ssi.local, group,
					0xFFFF, 0x0000, AIM_SSI_TYPE_GROUP,
					NULL)))
			return -ENOMEM;
	...
	/* Add that bad boy */
	aim_ssi_itemlist_add(&sess->ssi.local, name, parent->gid, 0xFFFF,
		AIM_SSI_TYPE_BUDDY, data);
...
faim_export int aim_ssi_addpermit(aim_session_t *sess, const char *name)
{
	...
	/* Add that bad boy */
	aim_ssi_itemlist_add(&sess->ssi.local, name, 0x0000, 0xFFFF,
		AIM_SSI_TYPE_PERMIT, NULL);
	...
	const char *group)
{
	...
	/* Find the buddy */
	if (!(del = aim_ssi_itemlist_finditem(sess->ssi.local, group, name,
				AIM_SSI_TYPE_BUDDY)))
		return -EINVAL;
	...
	if ((del = aim_ssi_itemlist_finditem(sess->ssi.local, group, NULL,
				AIM_SSI_TYPE_GROUP)) && (!del->data)) {
		aim_ssi_itemlist_del(&sess->ssi.local, del);
		...
		if ((del = aim_ssi_itemlist_find(sess->ssi.local, 0x0000,
					0x0000)) && (!del->data)) {
			aim_ssi_itemlist_del(&sess->ssi.local, del);
		}
	}
...
faim_export int aim_ssi_delpermit(aim_session_t *sess, const char *name)
{
	...
	/* Find the item */
	if (!(del = aim_ssi_itemlist_finditem(sess->ssi.local, NULL, name,
				AIM_SSI_TYPE_PERMIT)))
		return -EINVAL;
 ...
 * @return Return 0 if no errors, otherwise return the error number.
 */
faim_export int aim_ssi_movebuddy(aim_session_t *sess, const char *oldgn,
	const char *newgn, const char *sn)
{
	aim_ssi_addbuddy(sess, sn, newgn, aim_ssi_getalias(sess->ssi.local,
			oldgn, sn), NULL, NULL,
		aim_ssi_waitingforauth(sess->ssi.local, oldgn, sn));
	...
	const char *sn, const char *alias)
{
	...
	if (!(tmp = aim_ssi_itemlist_finditem(sess->ssi.local, gn, sn,
				AIM_SSI_TYPE_BUDDY)))
		return -EINVAL;
 ...
 * @return Return 0 if no errors, otherwise return the error number.
 */
faim_export int aim_ssi_rename_group(aim_session_t *sess, const char *oldgn,
	const char *newgn)
{
	...
	if (!(group = aim_ssi_itemlist_finditem(sess->ssi.local, oldgn, NULL,
				AIM_SSI_TYPE_GROUP)))
		return -EINVAL;
	...
	fu32_t vismask)
{
	...
	if ((tmp = aim_ssi_itemlist_finditem(sess->ssi.local, NULL, NULL,
				AIM_SSI_TYPE_PDINFO))) {
		aim_freetlvchain(&tmp->data);
		tmp->data = data;
	} else {
		tmp = aim_ssi_itemlist_add(&sess->ssi.local, NULL, 0x0000,
			0xFFFF, AIM_SSI_TYPE_PDINFO, data);
		aim_freetlvchain(&data);
	}
	...
	fu16_t iconsumlen)
{
	...
	aim_addtlvtochain_raw(&data, 0x00d5, (iconsumlen + 2) * sizeof(fu8_t),
		csumdata);

	/* This TLV is added to cache the icon. */
	aim_addtlvtochain_noval(&data, 0x0131);

	if ((tmp = aim_ssi_itemlist_finditem(sess->ssi.local, NULL, "1",
		...
		tmp = aim_ssi_itemlist_add(&sess->ssi.local, "1", 0x0000,
			0x51F4, AIM_SSI_TYPE_ICONINFO, data);
		aim_freetlvchain(&data);
	}
...
faim_export int aim_ssi_setpresence(aim_session_t *sess, fu32_t presence)
{
	...
	if ((tmp = aim_ssi_itemlist_finditem(sess->ssi.local, NULL, NULL,
		...
		tmp = aim_ssi_itemlist_add(&sess->ssi.local, NULL, 0x0000,
			0xFFFF, AIM_SSI_TYPE_PRESENCEPREFS, data);
		aim_freetlvchain(&data);
	}
...
faim_export int aim_ssi_reqrights(aim_session_t *sess)
{
	...
	return aim_genericreq_n(sess, conn, AIM_CB_FAM_SSI,
		AIM_CB_SSI_REQRIGHTS);
}

/*
 * Subtype 0x0003 - SSI Rights Information.
 */
static int parserights(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
...
faim_export int aim_ssi_reqdata(aim_session_t *sess)
{
	...
	return aim_genericreq_n_snacid(sess, conn, AIM_CB_FAM_SSI,
		AIM_CB_SSI_REQDATA);
}
 ...
 * 
 */
faim_export int aim_ssi_reqifchanged(aim_session_t *sess, time_t timestamp,
	fu16_t numitems)
{
	aim_conn_t *conn;
	aim_frame_t *fr;
	...
	snacid = aim_cachesnac(sess, AIM_CB_FAM_SSI, AIM_CB_SSI_REQIFCHANGED,
		0x0000, NULL, 0);

	aim_putsnac(&fr->data, AIM_CB_FAM_SSI, AIM_CB_SSI_REQIFCHANGED, 0x0000,
		snacid);
 ...
 */
static int parsedata(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
	...
	while (aim_bstream_empty(bs) > 4) {	/* last four bytes are timestamp */
		...
		aim_ssi_itemlist_add(&sess->ssi.official, name, gid, bid, type,
			data);
		free(name);
		aim_freetlvchain(&data);
	}
	...
	if (!(snac->flags & 0x0001)) {
		...
		for (cur = sess->ssi.official; cur; cur = cur->next)
			aim_ssi_itemlist_add(&sess->ssi.local, cur->name,
				cur->gid, cur->bid, cur->type, cur->data);

		sess->ssi.received_data = 1;

		if ((userfunc = aim_callhandler(sess, rx->conn, snac->family,
					snac->subtype)))
			ret = userfunc(sess, rx, fmtver, sess->ssi.numitems,
				sess->ssi.official, sess->ssi.timestamp);
	}
...
faim_export int aim_ssi_addmoddel(aim_session_t *sess)
{
	...
	if (!sess || !(conn = aim_conn_findbygroup(sess, AIM_CB_FAM_SSI))
		|| !sess->ssi.pending || !sess->ssi.pending->item)
		return -EINVAL;
	...
	snacid = aim_cachesnac(sess, AIM_CB_FAM_SSI, sess->ssi.pending->action,
		0x0000, NULL, 0);
	aim_putsnac(&fr->data, AIM_CB_FAM_SSI, sess->ssi.pending->action,
		0x0000, snacid);

	for (cur = sess->ssi.pending; cur; cur = cur->next) {
		aimbs_put16(&fr->data,
			cur->item->name ? strlen(cur->item->name) : 0);
		if (cur->item->name)
			aimbs_putraw(&fr->data, cur->item->name,
				strlen(cur->item->name));
		aimbs_put16(&fr->data, cur->item->gid);
		aimbs_put16(&fr->data, cur->item->bid);
		aimbs_put16(&fr->data, cur->item->type);
		aimbs_put16(&fr->data,
			cur->item->data ? aim_sizetlvchain(&cur->item->
				data) : 0);
		if (cur->item->data)
 ...
 */
static int parseadd(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
	...
	while (aim_bstream_empty(bs)) {
		...
		aim_ssi_itemlist_add(&sess->ssi.local, name, gid, bid, type,
			data);
		aim_ssi_itemlist_add(&sess->ssi.official, name, gid, bid, type,
			data);
		free(name);
 ...
 * XXX - It would probably be good for the client to actually do something when it gets this.
 */
static int parsemod(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
	...
	while (aim_bstream_empty(bs)) {
		...
		if ((item = aim_ssi_itemlist_find(sess->ssi.official, gid,
					bid))) {
			item->type = type;
			free(item->name);
			if (name) {
 ...
 * XXX - It would probably be good for the client to actually do something when it gets this.
 */
static int parsedel(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
 ...
 *
 */
static int parseack(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
	 ...
	 */
	for (cur = sess->ssi.pending; (cur && (cur->ack != 0xffff));
		cur = cur->next) {
		if (cur->item) {
			if (cur->ack) {
				...
				if (cur->action == AIM_CB_SSI_ADD) {
					...
					if (aim_ssi_itemlist_valid(sess->ssi.
							local, cur->item)) {
						if (cur->item->name) {
							cur->name =
								(char *)
								malloc((strlen
									(cur->item->name) + 1) * sizeof(char));
							strcpy(cur->name,
								cur->item->
								name);
						}
						aim_ssi_itemlist_del(&sess->ssi.
							local, cur->item);
					}
					cur->item = NULL;
						...
						if ((cur1 = aim_ssi_itemlist_find(sess->ssi.official, cur->item->gid, cur->item->bid))) {
							...
							if (cur1->name) {
								cur->item->
									...
									(char *)
									malloc(
									(strlen(cur1->name) + 1) * sizeof(char));
								strcpy(cur->
									item->
									name,
									cur1->
						...
						if ((cur1 = aim_ssi_itemlist_find(sess->ssi.official, cur->item->gid, cur->item->bid))) {
							...
							if (cur->item->name) {
								cur1->name =
									(char *)
									malloc(
									(strlen(cur->item->name) + 1) * sizeof(char));
								strcpy(cur1->
									name,
									cur->
									item->
 ...
 *
 */
static int parsedataunchanged(aim_session_t *sess, aim_module_t *mod,
...
faim_export int aim_ssi_sendauth(aim_session_t *sess, char *sn, char *msg)
{
	...
	if (!(fr = aim_tx_new(sess, conn, AIM_FRAMETYPE_FLAP, 0x02,
				10 + 1 + strlen(sn) + 2 + (msg ? strlen(msg) +
					1 : 0) + 2)))
		return -ENOMEM;

	snacid = aim_cachesnac(sess, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTH,
		0x0000, NULL, 0);
	aim_putsnac(&fr->data, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTH, 0x0000,
		snacid);
 ...
 */
static int receiveauthgrant(aim_session_t *sess, aim_module_t *mod,
	aim_frame_t *rx, aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
 ...
 *
 */
faim_export int aim_ssi_sendauthrequest(aim_session_t *sess, char *sn,
	char *msg)
{
	aim_conn_t *conn;
	aim_frame_t *fr;
	...
	snacid = aim_cachesnac(sess, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTHREQ,
		0x0000, NULL, 0);
	aim_putsnac(&fr->data, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTHREQ, 0x0000,
		snacid);
 ...
 */
static int receiveauthrequest(aim_session_t *sess, aim_module_t *mod,
	aim_frame_t *rx, aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
 ...
 *
 */
faim_export int aim_ssi_sendauthreply(aim_session_t *sess, char *sn,
	fu8_t reply, char *msg)
{
	aim_conn_t *conn;
	aim_frame_t *fr;
	aim_snacid_t snacid;
	...
	if (!(fr = aim_tx_new(sess, conn, AIM_FRAMETYPE_FLAP, 0x02,
				10 + 1 + strlen(sn) + 1 + 2 +
				(msg ? strlen(msg) + 1 : 0) + 2)))
		return -ENOMEM;

	snacid = aim_cachesnac(sess, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTHREP,
		0x0000, NULL, 0);
	aim_putsnac(&fr->data, AIM_CB_FAM_SSI, AIM_CB_SSI_SENDAUTHREP, 0x0000,
		snacid);
 ...
 */
static int receiveauthreply(aim_session_t *sess, aim_module_t *mod,
	aim_frame_t *rx, aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
 ...
 */
static int receiveadded(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
