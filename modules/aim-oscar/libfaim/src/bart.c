	...
	fu16_t iconlen)
{
	...
	if (!sess || !(conn = aim_conn_findbygroup(sess, 0x0010)) || !icon
		|| !iconlen)
		return -EINVAL;
 ...
 */
static int uploadack(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
...
faim_export int aim_bart_request(aim_session_t *sess, const char *sn,
	const fu8_t *iconcsum, fu16_t iconcsumlen)
{
	aim_conn_t *conn;
	aim_frame_t *fr;
	aim_snacid_t snacid;

	if (!sess || !(conn = aim_conn_findbygroup(sess, 0x0010)) || !sn
		|| !strlen(sn) || !iconcsum || !iconcsumlen)
		return -EINVAL;

	if (!(fr = aim_tx_new(sess, conn, AIM_FRAMETYPE_FLAP, 0x02,
				10 + 1 + strlen(sn) + 4 + 1 + iconcsumlen)))
		return -ENOMEM;
	...
	aimbs_putraw(&fr->data, (unsigned char *)sn, strlen(sn));
 ...
 */
static int parseicon(aim_session_t *sess, aim_module_t *mod, aim_frame_t *rx,
	aim_modsnac_t *snac, aim_bstream_t *bs)
{
	int ret = 0;
	aim_rxcallback_t userfunc;
	...
	if ((userfunc = aim_callhandler(sess, rx->conn, snac->family,
				snac->subtype)))
		ret = userfunc(sess, rx, sn, iconcsum, iconcsumlen, icon,
			iconlen);
