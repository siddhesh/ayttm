...
struct service SERVICE_INFO =
	{ "AIM-ICQ", -1, SERVICE_CAN_MULTIACCOUNT | SERVICE_CAN_GROUPCHAT,
		NULL };
/* End Module Exports */

static char *ay_aim_get_color(void)
{
	static char color[] = "#000088";
	return color;
}
...
static char *profile =
	"Visit the Ayttm website at <A HREF=\"http://ayttm.sf.net\">http://ayttm.sf.net/</A>.";
...
static eb_account *ay_aim_new_account(eb_local_account *ela,
	const char *account);
static void ay_aim_add_user(eb_account *account);
static void ay_aim_callback(void *data, int source,
...
static int ext_oscar_log(char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
...
ay_aim_callback(void *data, int source, eb_input_condition condition)
{
	...
	else if ((conn = oscar_find_chat_conn_by_source(alad,
				source)) != NULL) ;
...
static void
faim_cb_oscar_debug(aim_session_t *sess, int level, const char *format,
	va_list va)
{
	if (do_libfaim_debug) {
...
static int faim_cb_parse_login(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	ay_activity_bar_update_label(alad->login_activity_bar,
		"Sending password...");
...
static int faim_cb_parse_authresp(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	LOG(("faim_cb_parse_authresp () : Screen name : %s", info->sn))
		...
		/* Check for errors */
		if (info->errorcode || !info->bosip || !info->cookielen
		|| !info->cookie) {
		...
		case 0x1c:
			/* client too old */
			connect_error(alad,
				"The client version you are using is too old. Please upgrade at http://ayttm.sf.net/");
			break;
		default:
			connect_error(alad,
	...
	LOG(("New connection fd=%d", alad->conn->fd))
		...
		aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_ACK,
		AIM_CB_ACK_ACK, NULL, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_GEN, AIM_CB_GEN_MOTD,
		faim_cb_parse_motd, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SPECIAL,
		AIM_CB_SPECIAL_CONNERR, faim_cb_connerr, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SPECIAL,
		AIM_CB_SPECIAL_CONNINITDONE, faim_cb_conninitdone_bos, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_GEN,
		AIM_CB_GEN_SELFINFO, faim_cb_selfinfo, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SSI,
		AIM_CB_SSI_RIGHTSINFO, faim_cb_ssi_parserights, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SSI, AIM_CB_SSI_LIST,
		faim_cb_ssi_parselist, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SSI, AIM_CB_SSI_NOLIST,
		faim_cb_ssi_parselist, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_SSI, AIM_CB_SSI_SRVACK,
		faim_cb_ssi_parseack, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_LOC,
		AIM_CB_LOC_RIGHTSINFO, faim_cb_parse_locaterights, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_BUD,
		AIM_CB_BUD_RIGHTSINFO, faim_cb_parse_buddyrights, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_MSG,
		AIM_CB_MSG_PARAMINFO, faim_cb_icbmparaminfo, 0);

	/* FIXME : fix libfaim and use meaningful names:) */
	aim_conn_addhandler(sess, alad->conn, 0x0001, 0x001f,
		faim_cb_memrequest, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_BUD,
		AIM_CB_BUD_ONCOMING, faim_cb_parse_oncoming, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_BUD,
		AIM_CB_BUD_OFFGOING, faim_cb_parse_offgoing, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_MSG,
		AIM_CB_MSG_INCOMING, faim_cb_parse_incoming_im, 0);

	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_GEN,
		AIM_CB_GEN_REDIRECT, faim_cb_handle_redirect, 0);
#if 0
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_STS,
		AIM_CB_STS_SETREPORTINTERVAL, NULL, 0);
	...
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_MSG, AIM_CB_MSG_ERROR,
		eb_aim_msg_error, 0);
	aim_conn_addhandler(sess, alad->conn, AIM_CB_FAM_LOC,
		AIM_CB_LOC_USERINFO, (rxcallback_t) eb_aim_info_responce, 0);
#endif

	alad->input =
		eb_input_add(alad->conn->fd, EB_INPUT_READ | EB_INPUT_EXCEPTION,
		ay_aim_callback, ela);
...
static int faim_cb_connerr(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	LOG(("Disconnected.  Code is 0x%04x and msg is %s\n", code, msg))
		...
		if (code == 0x0001) {
			connect_error(alad,
				"You have been disconnected because you have signed on with this screen name at another location.");
		} else {
			connect_error(alad,
...
static int faim_cb_parse_genericerr(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	WARNING(("snac threw error (reason 0x%04hx: %s)\n", reason,
			(reason <
				msgerrreasonlen) ? msgerrreason[reason] :
			"unknown"))
...
static int faim_cb_ssi_parselist(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	alad = (struct eb_aim_local_account_data *)ela->
		protocol_local_account_data;
	...
	for (curitem = sess->ssi.local; curitem; curitem = curitem->next) {
		...
		case 0x0000:	/* Buddy */
			if (curitem->name) {
				char *group =
					aim_ssi_itemlist_findparentname(sess->
					ssi.local, curitem->name);
				char *alias =
					aim_ssi_getalias(sess->ssi.local, group,
					curitem->name);

				LOG(("[SSI] \\ Buddy %s, Group %s, Nick %s.",
						curitem->name, group, alias))

					ea =
					oscar_find_account_with_ela(curitem->
					name, ela, TRUE);

				if (ea) {
					aad = (struct eb_aim_account_data *)ea->
					...
					LOG(("       Cannot find the corresponding account"))
						ea =
						ay_aim_new_account(ela,
						curitem->name);
					add_unknown(ea);

					if (group != NULL && group[0]
						&& strcmp("~", group)) {
						if (!find_grouplist_by_name
							(group))
							add_group(group);
						move_contact(group,
							ea->account_contact);
	...
	for (node = alad->buddies; node; node = l_list_next(node)) {
		...
		if (!aad->on_server) {
			LOG(("[SSI] Need to add buddy %s on the server !",
					ea->handle))
				aim_ssi_addbuddy(sess, ea->handle,
...
static int faim_cb_parse_locaterights(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	LOG(("max away msg / signature len=%d\n", maxsiglen))
		...
		aim_locate_setprofile(sess, "us-ascii", profile, strlen(profile),	/* Profile */
	...
	struct aim_incomingim_ch2_args *args)
{
	...
	LOG(("Rendez vous with %s", userinfo->sn))
		...
		if (!args->info.chat.roominfo.name
			|| !args->info.chat.roominfo.exchange || !args->msg)
			return 1;
		...
		ocrd->name =
			(name) ? g_strdup(name) : g_strdup(args->info.chat.
			roominfo.name);
...
static int faim_cb_parse_incoming_im(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	case 1:{		/* Standard message */
			...
			va_end(ap);
			ret = oscar_incoming_im_chan1(sess, fr->conn, userinfo,
				args);
		} break;
	...
	case 2:{		/* Rendez vous */
			...
			va_end(ap);
			ret = oscar_incoming_im_chan2(sess, fr->conn, userinfo,
				args);
		} break;
...
static int faim_cb_handle_redirect(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	case 0xd:		/* ChatNav */
		LOG((" -> ChatNav"))
			tstconn =
			aim_newconn(&(alad->aimsess), AIM_CONN_TYPE_CHATNAV,
			NULL);
		...
		aim_conn_addhandler(&(alad->aimsess), tstconn,
			AIM_CB_FAM_SPECIAL, AIM_CB_SPECIAL_CONNERR,
			faim_cb_connerr, 0);
		aim_conn_addhandler(&(alad->aimsess), tstconn,
			AIM_CB_FAM_SPECIAL, AIM_CB_SPECIAL_CONNINITDONE,
			faim_cb_conninitdone_chatnav, 0);
		...
		if (ay_connection_connect(con,
				...
				oscar_chatnav_connect_status, NULL, ela) < 0) {
	...
	case 0xe:{		/* Chat */
			...
			LOG((" -> Chat"))
				...
				tstconn =
				aim_newconn(&(alad->aimsess),
				AIM_CONN_TYPE_CHAT, NULL);
			...
			con = ay_connection_new(host, port,
			...
			aim_sendcookie(sess, tstconn, redir->cookielen,
				redir->cookie);
...
faim_cb_conninitdone_chatnav(aim_session_t *sess, aim_frame_t *fr, ...)
{
	LOG(("faim_cb_conninitdone_chatnav()"))
		...
		aim_conn_addhandler(sess, fr->conn, 0x000d, 0x0001,
		faim_cb_parse_genericerr, 0);
	aim_conn_addhandler(sess, fr->conn, AIM_CB_FAM_CTN, AIM_CB_CTN_INFO,
		faim_cb_chatnav_info, 0);
...
static int faim_cb_chatnav_info(aim_session_t *sess, aim_frame_t *fr, ...)
{
	...
	case 0x0002:{
			...
			LOG(("chat info: Chat Rights:"))
				LOG(("chat info: \tMax Concurrent Rooms: %hhd",
					maxrooms))
				LOG(("chat info: \tExchange List: (%d total)",
					exchangecount))
			...
			while (alad->pending_rooms) {
				struct oscar_chat_room_data *ocrd =
					(struct oscar_chat_room_data *)alad->
					pending_rooms->data;
				LOG(("Creating room %s", ocrd->name))

					alad->rooms =
					l_list_append(alad->rooms, ocrd->ecr);
				aim_chatnav_createroom(sess, fr->conn,
					ocrd->name, ocrd->exchange);

				g_free(ocrd->name);
				alad->pending_rooms =
					l_list_remove(alad->pending_rooms,
					ocrd);
				g_free(ocrd);
			}
		}
	...
	case 0x0008:{
			char *fqcn, *name, *ck;
			fu16_t instance, flags, maxmsglen, maxoccupancy,
				unknown, exchange;
			fu8_t createperms;
			fu32_t createtime;
			...
			LOG(("created room: %s %hu %hu %hu %u %hu %hu %hhu %hu %s %s\n", fqcn, exchange, instance, flags, createtime, maxmsglen, maxoccupancy, createperms, unknown, name, ck))
				aim_chat_join(&(alad->aimsess), alad->conn,
				exchange, ck, instance);
		}
		break;
	default:
...
int eb_aim_msg_missed(struct aim_session_t *sess,
	struct command_rx_struct *command, ...)
{
	eb_account *ea =
		find_account_by_handle(&command->data[13],
...
int eb_aim_msg_error(struct aim_session_t *sess,
	struct command_rx_struct *command, ...)
{
...
static void oscar_chatnav_connect(AyConnection *fd, int error, void *data)
{
	...
	LOG(("oscar_chatnav_connect(): fd=%d, error=%d", alad->conn_chatnav->fd,
			error))
	...
	alad->input_chatnav =
		eb_input_add(alad->conn_chatnav->fd,
		EB_INPUT_READ | EB_INPUT_EXCEPTION, ay_aim_callback, account);
}
...
static void oscar_chat_connect(AyConnection *fd, int error, void *data)
{
	...
	ocr->input =
		eb_input_add(ocr->conn->fd, EB_INPUT_READ | EB_INPUT_EXCEPTION,
		ay_aim_callback, account);
}
...
static gboolean ay_aim_query_connected(eb_account *account)
{
...
/* Helper function */
static void connect_error(struct eb_aim_local_account_data *alad, char *msg)
{
	if (alad->login_activity_bar)
		ay_activity_bar_remove(alad->login_activity_bar);
...
static void ay_oscar_finish_login(const char *password, void *data)
{
	AyConnection *con;
	eb_local_account *account = (eb_local_account *)data;
	struct eb_aim_local_account_data *alad =
	...
	snprintf(buf, sizeof(buf), "Logging in to AIM account: %s",
		account->handle);
	alad->login_activity_bar =
		ay_activity_bar_add(buf, ay_aim_cancel_connect, account);
	...
	aim_conn_addhandler(&(alad->aimsess), alad->conn,
		AIM_CB_FAM_SPECIAL, AIM_CB_SPECIAL_CONNERR, faim_cb_connerr, 0);

	alad->conn->status |= AIM_CONN_STATUS_INPROGRESS;

	con = ay_connection_new(FAIM_LOGIN_SERVER, FAIM_LOGIN_PORT,
		AY_CONNECTION_TYPE_PLAIN);
	...
	LOG(("Requesting connection with screename %s\n", account->handle))
		aim_request_login(&(alad->aimsess), alad->conn,
		account->handle);
}
...
static void ay_aim_login(eb_local_account *account)
{
	...
	LOG(("ay_aim_login: Incrementing ref_count to %i\n", ref_count))
		...
		if (alad->prompt_password || !alad->password
		|| !strlen(alad->password)) {

		snprintf(buf, sizeof(buf), "AIM password for: %s",
			account->handle);
		do_password_input_window(buf, "", ay_oscar_finish_login,
			account);
	} else {
 ...
 * buddy offline.
 */
static void make_buddy_offline(void *b, void *n)
{
	eb_account *user;
	struct eb_aim_account_data *aad;
...
static LList *ay_aim_write_local_config(eb_local_account *ela)
{
	return eb_input_to_value_pair(ela->prefs);
}

static void oscar_init_account_prefs(eb_local_account *ela)
{
	struct eb_aim_local_account_data *alad =
		(struct eb_aim_local_account_data *)ela->
		protocol_local_account_data;
...
static eb_local_account *ay_aim_read_local_config(LList *pairs)
{
...
static eb_account *ay_aim_read_config(eb_account *ea, LList *config)
{
...
static void ay_aim_add_user(eb_account *account)
{
	...
	if ((alad->status != AIM_OFFLINE) && (!alad->listsyncing)) {
		LOG(("Adding the buddy to the remote buddy list"))
			...
			aim_ssi_addbuddy(&(alad->aimsess), name, group, nick,
			NULL, NULL, 0);
	}
}
...
static void ay_aim_del_user(eb_account *account)
{
	eb_local_account *ela = NULL;
	struct eb_aim_local_account_data *alad = NULL;
	...
	if ((alad->status != AIM_OFFLINE) && (!alad->listsyncing)) {
		LOG(("suppression ### %s ..... %s", account->handle,
	...
	eb_account *account_to, gchar *message)
{
	...
	struct eb_aim_local_account_data *alad =
		(struct eb_aim_local_account_data *)account_from->
		protocol_local_account_data;
...
static gint ay_aim_get_current_state(eb_local_account *account)
{
	struct eb_aim_local_account_data *alad;
	alad = (struct eb_aim_local_account_data *)account->
		protocol_local_account_data;
...
static void ay_aim_set_current_state(eb_local_account *account, gint state)
{
	char *awaymsg;
	struct eb_aim_local_account_data *alad;
	alad = (struct eb_aim_local_account_data *)account->
	...
	case AIM_ONLINE:
		...
		aim_locate_setprofile(&(alad->aimsess), NULL, NULL, 0,	/* Profile */
			NULL, "", 0,	/* Away message */
			caps_aim);	/* What we can do */
		break;
	...
	case AIM_AWAY:
		...
		aim_locate_setprofile(&(alad->aimsess),
			NULL, NULL, 0,
			"iso-8859-1", awaymsg, strlen(awaymsg), caps_aim);
		break;
...
static void *ay_aim_get_status_pixbuf(eb_account *account)
{
	struct eb_aim_account_data *aad;
...
static const gchar *ay_aim_get_status_string(eb_account *account)
{
...
static char *ay_aim_check_login(const char *user, const char *pass)
{
	return NULL;
}
...
oscar_create_room(eb_local_account *account, struct oscar_chat_room_data *ocrd)
{
	...
	if ((cur = aim_getconn_type(&(alad->aimsess), AIM_CONN_TYPE_CHATNAV))) {
		...
		aim_chatnav_createroom(&(alad->aimsess), cur, ocrd->name,
			ocrd->exchange);
		g_free(ocrd->name);
		g_free(ocrd);
	} else {
		LOG(("chatnav does not exist, opening chatnav"))
			alad->pending_rooms =
			l_list_append(alad->pending_rooms, ocrd);
		aim_reqservice(&(alad->aimsess), alad->conn,
...
static void ay_oscar_decline_invite(eb_local_account *account, void *data)
{
	struct oscar_chat_room_data *ocrd = (struct oscar_chat_room_data *)data;
	...
	eb_local_account *ela, gint should_update)
{
	...
	if (result) {
		if (should_update && (strcmp(handle, result->handle) != 0)) {
			WARNING(("Updating contact list from %s to %s",
					result->handle, handle))
				strncpy(result->handle, handle,
				sizeof(result->handle) - 1);
			write_contact_list();
		}
	}

	if (result) {
		LOG(("oscar_find_account_with_ela(): %s => %s", handle,
				result->account_contact->nick))
	} else {
