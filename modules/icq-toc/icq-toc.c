...
struct service SERVICE_INFO =
	{ "ICQ", -1, SERVICE_CAN_GROUPCHAT | SERVICE_CAN_ICONVERT, NULL };
/* End Module Exports */

static char *eb_toc_get_color(void)
{
	static char color[] = "#000088";
	return color;
}

static eb_chat_room *eb_icq_make_chat_room(char *name,
...
static eb_account *eb_icq_new_account(eb_local_account *ela,
	const char *account);
	...
	for (node = accounts; node; node = node->next) {
		...
		if (ela->service_id == SERVICE_INFO.protocol_id) {
			struct eb_icq_local_account_data *alad =
				(struct eb_icq_local_account_data *)ela->
	...
	char *cookie, char *filename)
{
	...
	g_snprintf(message, 1024,
		_
		("icq user %s would like to\nsend you the file\n%s\ndo you want to accept?"),
		nick, filename);

	eb_do_dialog(message, _("Incoming icq File Request"),
		eb_icq_process_file_request, eafr);
...
static void eb_icq_toc_chat_im_in(toc_conn *conn, char *id, char *user,
	char *message)
{
	eb_chat_room *ecr = find_chat_room_by_id(id);
	eb_account *ea =
...
static void eb_icq_new_user(toc_conn *conn, char *group, char *f_handle)
{
	...
	if (!ea) {
		...
		if (alad)
			alad->icq_buddies =
				l_list_append(alad->icq_buddies, handle);
...
static void eb_icq_parse_incoming_im(toc_conn *conn, char *user, char *message)
{
	...
	eb_debug(DBG_TOC, "eb_icq_parse_incomming_im %d %d, %d %d\n", conn->fd,
		conn->seq_num, alad->conn->fd, alad->conn->seq_num);
...
static void eb_icq_add_user(eb_account *account)
{
	...
	struct eb_icq_local_account_data *alad =
		account->ela ? (struct eb_icq_local_account_data *)account->
	...
	for (node = accounts; node; node = node->next) {
		...
		if (ela && ela->connected
			&& ela->service_id == account->service_id) {
			struct eb_icq_local_account_data *alad =
				ela->protocol_local_account_data;
...
static void ay_icq_cancel_connect(void *data)
{
	...
	struct eb_icq_local_account_data *alad;
	alad = (struct eb_icq_local_account_data *)ela->
		protocol_local_account_data;
...
static void eb_icq_finish_login(const char *password, void *data)
{
	...
	eb_local_account *account = (eb_local_account *)data;
	alad = (struct eb_icq_local_account_data *)account->
		protocol_local_account_data;

	snprintf(buff, sizeof(buff), _("Logging in to ICQ account: %s"),
		account->handle);
	alad->activity_tag =
		ay_activity_bar_add(buff, ay_icq_cancel_connect, account);
...
static void eb_icq_login(eb_local_account *account)
{
	...
	if (alad->prompt_password || !alad->password || !strlen(alad->password)) {
		snprintf(buff, sizeof(buff), _("ICQ password for: %s"),
			account->handle);
		do_password_input_window(buff, "", eb_icq_finish_login,
			account);
	} else {
...
static void eb_icq_logged_in(toc_conn *conn)
{
	...
	if (alad->conn->fd == -1) {
		...
		if (icq_fallback_ports[icq_last_fallback] != 0) {
			...
			ay_do_error(_("ICQ Error"),
				_
				("Cannot connect to ICQ due to network problem."));
			should_fallback = 0;
			icq_last_fallback = 0;
		}
		return;
	...
	ref_count++;
	alad->input =
		eb_input_add(alad->conn->fd, EB_INPUT_READ, eb_icq_callback,
		alad);

	alad->keep_alive =
		eb_timeout_add((guint32) 60000, eb_icq_keep_alive,
		(gpointer) alad);
	...
	icqtoc_add_buddy(alad->conn, ela->handle, "Unknown");
...
{
	int tag =
		proxy_connect_host(host, port, cb, data,
		(void *)ay_toc_connect_status);
...
{
	struct eb_icq_local_account_data *plad =
		(struct eb_icq_local_account_data *)account_from->
		protocol_local_account_data;
	char *message2 = strdup(message);
...
static eb_local_account *eb_icq_read_local_config(LList *pairs)
{
	...
	struct eb_icq_local_account_data *ala =
		g_new0(struct eb_icq_local_account_data, 1);
...
static int eb_icq_get_current_state(eb_local_account *account)
{
	...
	eb_debug(DBG_TOC, "eb_icq_get_current_state: %i %i\n",
		eb_services[account->service_id].protocol_id,
		SERVICE_INFO.protocol_id);
	assert(eb_services[account->service_id].protocol_id ==
		SERVICE_INFO.protocol_id);
...
//      assert( eb_services[account->service_id].protocol_id == SERVICE_INFO.protocol_id );
	...
	case ICQ_AWAY:
		...
		if (is_away) {
			...
			icqtoc_set_away(alad->conn,
				_("User is currently away"));
		break;
	case ICQ_OFFLINE:
...
static void eb_icq_real_change_group(eb_account *ea, const char *old_group,
	const char *new_group)
{
	...
	if (ea->ela && ea->ela->connected
		&& ea->ela->service_id == ea->service_id) {
...
{
	eb_icq_real_change_group(ea, ea->account_contact->group->name,
		new_group);
}
...
static void eb_icq_rename_group(eb_local_account *ela, const char *old_group,
	const char *new_group)
{
	LList *l;
	...
	for (l = alad->icq_buddies; l; l = l->next) {
		...
		if (ea)
			eb_debug(DBG_TOC,
				"checking if we should move %s from %s\n",
...
{
	struct eb_icq_local_account_data *alad;
	alad = (struct eb_icq_local_account_data *)from->
		protocol_local_account_data;
