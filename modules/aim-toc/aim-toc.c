...
struct service SERVICE_INFO =
	{ "AIM", -1,
		SERVICE_CAN_MULTIACCOUNT | SERVICE_CAN_GROUPCHAT |
...
static eb_chat_room *eb_aim_make_chat_room(char *name,
...
static eb_account *eb_aim_new_account(eb_local_account *ela,
	const char *account);
	...
	char *user, int online)
{
	...
	if (online) {
		eb_account *ea =
			...
			aim_find_local_account_by_conn(conn));
		if (ea) {
...
static void eb_aim_callback(void *data, int source,
	...
	char *cookie, char *filename)
{
	...
	g_snprintf(message, 1024,
		_
		("AIM user %s would like to\nsend you the file\n%s\ndo you want to accept?"),
		nick, filename);

	eb_do_dialog(message, _("Incoming AIM File Request"),
		eb_aim_process_file_request, eafr);
...
static void eb_aim_oncoming_buddy(toc_conn *conn, char *user, int online,
...
static void eb_aim_user_info(toc_conn *conn, char *user, char *message)
{
	...
	if (sender == NULL) {
		...
		struct eb_aim_account_data *aad =
			g_new0(struct eb_aim_account_data, 1);
...
static void eb_aim_new_user(toc_conn *conn, char *group, char *f_handle)
{
	...
	if (!ea) {
		...
		if (alad)
			alad->aim_buddies =
				l_list_append(alad->aim_buddies, handle);
...
static void eb_aim_add_user(eb_account *account)
{
	LList *node;
	struct eb_aim_local_account_data *alad =
		account->ela ? (struct eb_aim_local_account_data *)account->
...
static void eb_aim_finish_login(const char *password, void *data)
{
	...
	alad->activity_tag =
		ay_activity_bar_add(buff, ay_aim_cancel_connect, account);
...
static void eb_aim_login(eb_local_account *account)
{
	...
	if (alad->prompt_password || !alad->password || !strlen(alad->password)) {
		snprintf(buff, sizeof(buff), _("AIM password for: %s"),
			account->handle);
		do_password_input_window(buff, "", eb_aim_finish_login,
			account);
	} else {
...
static void eb_aim_logged_in(toc_conn *conn)
{
	...
	if (alad->conn->fd == -1) {
		...
		if (aim_fallback_ports[aim_last_fallback] != 0) {
			...
			ay_do_error(_("AIM Error"),
				_
				("Cannot connect to AIM due to network problem."));
			should_fallback = 0;
			aim_last_fallback = 0;
		}
		return;
	...
	ref_count++;
	alad->input =
		eb_input_add(alad->conn->fd, EB_INPUT_READ, eb_aim_callback,
		alad);

	alad->keep_alive =
		eb_timeout_add((guint32) 60000, eb_aim_keep_alive,
		(gpointer) alad);
	...
	toc_add_buddy(alad->conn, ela->handle, "Unknown");
...
{
	struct eb_aim_local_account_data *plad =
...
static void eb_aim_rename_group(eb_local_account *ela, const char *old_group,
	const char *new_group)
{
	LList *l;
	struct eb_aim_local_account_data *alad =
...
{
	struct eb_aim_local_account_data *alad;
	alad = (struct eb_aim_local_account_data *)from->
		protocol_local_account_data;
...
static int eb_aim_handle_url(const char *url)
{
	...
	if (!strncmp("aim:", url, 4)) {
		...
		if (!strncmp(action, "goim?", 5)) {
			...
			if (!find_account_by_handle(screenname,
					SERVICE_INFO.protocol_id))
				eb_aim_new_user(NULL, _("Unknown"), screenname);

			if (eb_send_message(screenname, message,
					SERVICE_INFO.protocol_id))
				goto ok;
			else
				goto err;
 ...
 err:
	ay_do_error(_("AIM Error"),
		_("This URL isn't supported by AIM module."));
 silent_err:
	res = 0;
 ok:
	if (str)
