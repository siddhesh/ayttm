...
static void toc_get_file_data(gpointer data, int source,
...
static void toc_get_file(const char *ip, short port, const char *cookie,
	const char *filename)
{
	int fd;
	char buff[2048];
	...
	conn->progress =
		toc_begin_file_receive(filename, ntohl(*((long *)(buff + 22))));
...
//              time_t signon;
		...
		update_user_status(conn, user, online, idle_time, atoi(evil),
			unavailable);
		...
		if (!strcmp(uuid, SEND_FILE_UUID)) {
			toc_file_offer(conn, nick, ip, atoi(port), cookie,
				filename);
...
void toc_callback_handler(gpointer data, int source,
	eb_input_condition condition)
{
	toc_callback(data);
}
