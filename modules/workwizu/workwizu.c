...
struct service SERVICE_INFO =
	{ "Workwizu", -1, FALSE, TRUE, FALSE, TRUE, NULL };
/* End Module Exports */
...
void sock_write(int sock, char *buf)
{
	...
	if (write(sock, buf, strlen(buf)) == -1) {
		perror("write");
		account =
			find_local_account_by_handle(my_user->username,
			SERVICE_INFO.protocol_id);
		ay_do_error(_("Workwizu Error"),
			_("Connection closed (can't write)"));
...
void workwizu_answer_challenge(int sock)
{
	...
	buf = g_strdup_printf("%s%%%s%%0%%0%%0%%%s\n",
		my_user->username, compute_challenge(), owner);
	sock_write(sock, buf);
	g_free(buf);
...
static void parse_stats(int from, char *stats)
{
	...
	eb_account *ea =
		find_account_by_handle(sfrom, SERVICE_INFO.protocol_id);
	wwz_user *user;
...
void get_message(int from, int to, char *data)
{
	...
	if (to > CHANNEL || to < NOTDEFINED)
		...
		wwz_account_data *wad =
			(wwz_account_data *)ela->protocol_local_account_data;
...
void workwizu_handle_incoming(int sock, int readable, int writable)
{
	...
	case CONN_WAITING_USERINFO:
		...
		if (line == NULL) {
			ay_do_error(_("Workwizu Error"),
				_("Bad authentication."));
			eb_workwizu_logout(account);
			return;
		}
		if (strncmp(line, my_user->username, strlen(my_user->username))) {
			eb_debug(DBG_WWZ,
				"got normal packet before auth result :(\n");
			parse_packet(line);
			free(line);
			return;
		}
		...
		if (my_user->is_driver)
			send_my_packet(sock,
				...
				EXCEPTME, my_user->username);

		free(line);
		my_user->statssender = eb_timeout_add(10 * 60 * 1000,	/* 10 minutes */
	...
	case CONN_ESTABLISHED:
		...
		if (line == NULL) {
			char *buf =
				g_strdup_printf(_("Connection to %s lost."),
				server);
...
void eb_workwizu_incoming(void *data, int source, eb_input_condition condition)
{
	if (!(condition & EB_INPUT_EXCEPTION)) {
		workwizu_handle_incoming(source,
			condition & EB_INPUT_READ, condition & EB_INPUT_WRITE);
	}
}
...
void register_sock(int s, int reading, int writing)
{
	...
	for (a = 0; a < 20; a++) {
		if (tags[a].fd == -1) {
			...
			if (reading) {
				tags[a].tag_r =
					eb_input_add(s, EB_INPUT_READ,
					eb_workwizu_incoming, NULL);
			}

			if (writing) {
				tags[a].tag_w =
					eb_input_add(s, EB_INPUT_WRITE,
					eb_workwizu_incoming, NULL);
			}
...
void unregister_sock(int s)
{
	...
	for (a = 0; a < 20; a++) {
		if (tags[a].fd == s) {
			if (tags[a].tag_r != -1) {
				eb_input_remove(tags[a].tag_r);
			}
			if (tags[a].tag_w != -1) {
				eb_input_remove(tags[a].tag_w);
			}
			tags[a].fd = -1;
...
void eb_workwizu_connected(int fd, int error, void *data)
{
	...
	wwz_account_data *wad =
		(wwz_account_data *)account->protocol_local_account_data;
...
void eb_workwizu_login(eb_local_account *account)
{
	...
	snprintf(buff, sizeof(buff), _("Logging in to Workwizu account: %s"),
		account->handle);
	wad->activity_tag =
		ay_activity_bar_add(buff, ay_wwz_cancel_connect, account);
	...
	if ((res = proxy_connect_host(server, atoi(port),
				(ay_socket_callback) eb_workwizu_connected,
				account, NULL)) < 0) {
...
void eb_workwizu_logout(eb_local_account *account)
{
	...
	while (l && l->data) {
		...
		if (l)
			ea = (eb_account *)find_account_by_handle((char *)l->
				data, SERVICE_INFO.protocol_id);
		if (ea) {
			buddy_logoff(ea);
			buddy_update_status(ea);
			eb_workwizu_del_user(ea);
...
{
	wwz_account_data *wad =
		(wwz_account_data *)account_from->protocol_local_account_data;
	...
	eb_account *account_to)
{
	...
	my_user->typing_handler =
		eb_timeout_add(5000, (eb_timeout_function) reset_typing,
		GINT_TO_POINTER(wad->sock));
	return 4;
}
...
void eb_workwizu_join_chat_room(eb_chat_room *room)
{
	...
	room->connected = TRUE;
	room->fellows = NULL;
	for (all = wwz_contacts; all != NULL && all->data != NULL;
		all = all->next) {
		eb_account *ea =
			(eb_account *)find_account_by_handle((char *)all->data,
			SERVICE_INFO.protocol_id);
		if (!ea)
			return;
