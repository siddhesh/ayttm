...
static int defineAttribute(ELEMENT_TYPE *type, ATTRIBUTE_ID *, int isCdata,
	const XML_Char *dfltValue);
static enum XML_Error storeAttributeValue(XML_Parser parser, const ENCODING *,
	int isCdata, const char *, const char *, STRING_POOL *);
...
XML_Parser XML_ParserCreate(const XML_Char *encodingName)
{
	...
	if (!dtdInit(&dtd) || !atts || !dataBuf || (encodingName
			&& !protocolEncodingName)) {
...
XML_Parser XML_ExternalEntityParserCreate(XML_Parser oldParser,
	const XML_Char *context, const XML_Char *encodingName)
{
	...
	parser = (ns ? XML_ParserCreateNS(encodingName, namespaceSeparator)
...
void XML_SetNamespaceDeclHandler(XML_Parser parser,
	XML_StartNamespaceDeclHandler start, XML_EndNamespaceDeclHandler end)
{
...
int XML_Parse(XML_Parser parser, const char *s, int len, int isFinal)
{
	if (len == 0) {
		...
		errorCode = processor(parser, bufferPtr, parseEndPtr =
			bufferEnd, 0);
		...
		positionPtr = s;
		if (isFinal) {
			errorCode = processor(parser, s, parseEndPtr =
				s + len, 0);
			if (errorCode == XML_ERROR_NONE)
				return 1;
			eventEndPtr = eventPtr;
...
int XML_ParseBuffer(XML_Parser parser, int len, int isFinal)
{
	...
	if (errorCode == XML_ERROR_NONE) {
		if (!isFinal)
			XmlUpdatePosition(encoding, positionPtr, bufferPtr,
				&position);
		return 1;
	} else {
		eventEndPtr = eventPtr;
	...
	case XML_TOK_XML_DECL:
		{
			enum XML_Error result =
				processXmlDecl(parser, 1, start, next);
			if (result != XML_ERROR_NONE)
				return result;
			start = next;
		}
		...
		*eventEndPP = next;
		...
		case XML_TOK_START_TAG_NO_ATTS:
			{
				...
				if (startElementHandler) {
					...
					*toPtr = XML_T('\0');
					result = storeAtts(parser, enc, s,
						&(tag->name), &(tag->bindings));
					if (result)
						return result;
					startElementHandler(handlerArg,
						tag->name.str,
						(const XML_Char **)atts);
					poolClear(&tempPool);
				} else {
		...
		case XML_TOK_EMPTY_ELEMENT_NO_ATTS:
			if (startElementHandler || endElementHandler) {
				...
				TAG_NAME name;
				name.str =
					poolStoreString(&tempPool, enc, rawName,
					rawName + XmlNameLength(enc, rawName));
				if (!name.str)
					return XML_ERROR_NO_MEMORY;
				poolFinish(&tempPool);
				result = storeAtts(parser, enc, s, &name,
					&bindings);
				if (result)
					return result;
				poolFinish(&tempPool);
			...
			if (tagLevel == 0)
				return epilogProcessor(parser, next, end,
					nextPtr);
			break;
		case XML_TOK_END_TAG:
			if (tagLevel == startTagLevel)
				...
				if (tagLevel == 0)
					return epilogProcessor(parser, next,
						end, nextPtr);
			}
			break;
		...
		case XML_TOK_CDATA_SECT_OPEN:
			{
				...
				/* Suppose you doing a transformation on a document that involves
				   ...
				   easier to let the user deal with this. */
					...
					reportDefault(parser, enc, s, next);
				result = doCdataSection(parser, enc, &next, end,
					nextPtr);
				if (!next) {
	...
	const char *s, TAG_NAME *tagNamePtr, BINDING **bindingsPtr)
{
	...
	if (tagNamePtr) {
		elementType =
			(ELEMENT_TYPE *)lookup(&dtd.elementTypes,
			tagNamePtr->str, 0);
		if (!elementType) {
			tagNamePtr->str =
				poolCopyString(&dtd.pool, tagNamePtr->str);
			if (!tagNamePtr->str)
				return XML_ERROR_NO_MEMORY;
			elementType =
				(ELEMENT_TYPE *)lookup(&dtd.elementTypes,
	...
	for (i = 0; i < n; i++) {
		...
		if (!atts[i].normalized) {
			...
			if (attId->maybeTokenized) {
				...
				for (j = 0; j < nDefaultAtts; j++) {
					if (attId ==
						elementType->defaultAtts[j].
						id) {
						isCdata =
							elementType->
							defaultAtts[j].isCdata;
						break;
					}
				}
			}
		...
		if (attId->prefix && tagNamePtr) {
			if (attId->xmlns) {
				if (!addBinding(parser, attId->prefix, attId,
	...
	if (nPrefixes) {
		for (; i < attIndex; i += 2) {
			if (appAtts[i][-1] == 2) {
				...
				id = (ATTRIBUTE_ID *)lookup(&dtd.attributeIds,
					appAtts[i], 0);
	...
	do {
		if (i == binding->uriAlloc) {
			binding->uri =
				realloc(binding->uri, binding->uriAlloc *= 2);
			if (!binding->uri)
				return XML_ERROR_NO_MEMORY;
		}
...
enum XML_Error doCdataSection(XML_Parser parser,
	const ENCODING *enc,
	const char **startPtr, const char *end, const char **nextPtr)
{
	...
	if (unknownEncodingHandler) {
		...
		if (unknownEncodingHandler(unknownEncodingHandlerData,
			...
			enc = (ns
				...
				info.map, info.convert, info.data);
			if (enc) {
	...
	const char *s, const char *end, const char **nextPtr)
{
	for (;;) {
		...
		case XML_ROLE_ATTLIST_ELEMENT_NAME:
			{
				const XML_Char *name =
					poolStoreString(&dtd.pool, encoding, s,
					next);
				if (!name)
					return XML_ERROR_NO_MEMORY;
				declElementType =
					(ELEMENT_TYPE *)lookup(&dtd.
					elementTypes, name,
					sizeof(ELEMENT_TYPE));
				if (!declElementType)
					return XML_ERROR_NO_MEMORY;
				if (declElementType->name != name)
					...
					poolFinish(&dtd.pool);
					if (!setElementTypePrefix(parser,
							declElementType))
						return XML_ERROR_NO_MEMORY;
				}
				break;
		...
		case XML_ROLE_ATTRIBUTE_NAME:
			declAttributeId =
				getAttributeId(parser, encoding, s, next);
			if (!declAttributeId)
				return XML_ERROR_NO_MEMORY;
		...
		case XML_ROLE_REQUIRED_ATTRIBUTE_VALUE:
			if (dtd.complete
				&& !defineAttribute(declElementType,
					declAttributeId, declAttributeIsCdata,
					0))
				return XML_ERROR_NO_MEMORY;
			break;
		...
		case XML_ROLE_FIXED_ATTRIBUTE_VALUE:
			{
				...
				enum XML_Error result
					=
					storeAttributeValue(parser, encoding,
		...
		case XML_ROLE_ENTITY_VALUE:
			{
				enum XML_Error result =
					storeEntityValue(parser, s, next);
				if (result != XML_ERROR_NONE)
					return result;
			}
			break;
		case XML_ROLE_ENTITY_SYSTEM_ID:
			if (declEntity) {
				declEntity->systemId =
					poolStoreString(&dtd.pool, encoding,
					s + encoding->minBytesPerChar,
					next - encoding->minBytesPerChar);
		...
		case XML_ROLE_GENERAL_ENTITY_NAME:
			{
				...
				name = poolStoreString(&dtd.pool, encoding, s,
					next);
				if (!name)
					return XML_ERROR_NO_MEMORY;
				if (dtd.complete) {
		...
		case XML_ROLE_NOTATION_NAME:
			...
			if (notationDeclHandler) {
				declNotationName =
					poolStoreString(&tempPool, encoding, s,
					next);
				if (!declNotationName)
					return XML_ERROR_NO_MEMORY;
				poolFinish(&tempPool);
		...
		case XML_ROLE_NOTATION_NO_SYSTEM_ID:
			if (declNotationPublicId && notationDeclHandler) {
				...
				notationDeclHandler(handlerArg,
					declNotationName,
					dtd.base, 0, declNotationPublicId);
			}
			poolClear(&tempPool);
			break;
		case XML_ROLE_ERROR:
		...
		case XML_ROLE_NONE:
			...
			case XML_TOK_PI:
				eventPtr = s;
				eventEndPtr = next;
				if (!reportProcessingInstruction(parser,
						encoding, s, next))
					return XML_ERROR_NO_MEMORY;
				break;
			case XML_TOK_COMMENT:
	...
	const char *s, const char *end, const char **nextPtr)
{
	...
	for (;;) {
		...
		case XML_TOK_PROLOG_S:
			...
			break;
		case XML_TOK_PI:
			if (!reportProcessingInstruction(parser, encoding, s,
					next))
				return XML_ERROR_NO_MEMORY;
			break;
		case XML_TOK_COMMENT:
		...
		case XML_TOK_ENTITY_REF:
			{
				...
				if (!entity) {
					...
					enum XML_Error result;
					const XML_Char *textEnd =
						entity->textPtr +
						entity->textLen;
					entity->open = 1;
					result = appendAttributeValue(parser,
						internalEnc, isCdata,
						(char *)entity->textPtr,
						(char *)textEnd, pool);
					entity->open = 0;
	...
	const char *entityTextPtr, const char *entityTextEnd)
{
	...
	for (;;) {
		const char *next;
		int tok =
			XmlEntityValueTok(encoding, entityTextPtr,
			entityTextEnd, &next);
		switch (tok) {
		case XML_TOK_PARAM_ENTITY_REF:
		...
		case XML_TOK_CHAR_REF:
			{
				XML_Char buf[XML_ENCODE_MAX];
				int i;
				int n = XmlCharRefNumber(encoding,
					entityTextPtr);
				if (n < 0) {
					eventPtr = entityTextPtr;
					return XML_ERROR_BAD_CHAR_REF;
	...
	const char *start, const char *end)
{
	...
	data = poolStoreString(&tempPool, enc,
		XmlSkipS(enc, tem), end - enc->minBytesPerChar * 2);
	if (!data)
		return 0;
	normalizeLines(data);
	...
	const char *end)
{
	if (MUST_CONVERT(enc, s)) {
		...
		do {
			...
			*eventEndPP = s;
			defaultHandler(handlerArg, dataBuf,
				dataPtr - (ICHAR *)dataBuf);
			*eventPP = s;
		} while (s != end);
	} else
	...
	for (name = elementType->name; *name; name++) {
		if (*name == XML_T(':')) {
			...
			if (!poolAppendChar(&dtd.pool, XML_T('\0')))
				return 0;
			prefix = (PREFIX *)lookup(&dtd.prefixes,
	...
	if (id->name != name)
		...
		if (!ns) ;
			...
			if (name[5] == '\0')
				...
				id->prefix =
					(PREFIX *)lookup(&dtd.prefixes,
					name + 6, sizeof(PREFIX));
			id->xmlns = 1;
		} else {
			int i;
			for (i = 0; name[i]; i++) {
...
int setContext(XML_Parser parser, const XML_Char *context)
{
	...
	while (*context != XML_T('\0')) {
		if (*s == CONTEXT_SEP || *s == XML_T('\0')) {
			...
			if (!poolAppendChar(&tempPool, XML_T('\0')))
				return 0;
			e = (ENTITY *)lookup(&dtd.generalEntities,
				poolStart(&tempPool), 0);
			if (e)
				e->open = 1;
			if (*s != XML_T('\0'))
				s++;
			...
			if (poolLength(&tempPool) == 0)
				...
				if (!poolAppendChar(&tempPool, XML_T('\0')))
					return 0;
				prefix = (PREFIX *)lookup(&dtd.prefixes,
					poolStart(&tempPool), sizeof(PREFIX));
				if (!prefix)
					return 0;
...
static int dtdCopy(DTD *newDtd, const DTD *oldDtd)
{
	...
	if (oldDtd->base) {
		const XML_Char *tem =
			poolCopyString(&(newDtd->pool), oldDtd->base);
		if (!tem)
			return 0;
		newDtd->base = tem;
	}
	...
	for (;;) {
		...
		const ELEMENT_TYPE *oldE =
			(ELEMENT_TYPE *)hashTableIterNext(&iter);
		if (!oldE)
			break;
		...
		for (i = 0; i < newE->nDefaultAtts; i++) {
			newE->defaultAtts[i].id =
				(ATTRIBUTE_ID *)lookup(&(newDtd->attributeIds),
				oldE->defaultAtts[i].id->name, 0);
			newE->defaultAtts[i].isCdata =
				oldE->defaultAtts[i].isCdata;
			if (oldE->defaultAtts[i].value) {
				newE->defaultAtts[i].value =
					poolCopyString(&(newDtd->pool),
					oldE->defaultAtts[i].value);
	...
	for (;;) {
		...
		if (!name)
			return 0;
		newE = (ENTITY *)lookup(&(newDtd->generalEntities), name,
			sizeof(ENTITY));
		if (!newE)
			return 0;
		if (oldE->systemId) {
			const XML_Char *tem =
				poolCopyString(&(newDtd->pool), oldE->systemId);
			if (!tem)
				return 0;
			newE->systemId = tem;
			if (oldE->base) {
				...
				tem = poolCopyString(&(newDtd->pool),
					oldE->base);
				if (!tem)
					return 0;
				newE->base = tem;
			...
			const XML_Char *tem =
				poolCopyStringN(&(newDtd->pool), oldE->textPtr,
				oldE->textLen);
			if (!tem)
				return 0;
			newE->textPtr = tem;
		...
		if (oldE->notation) {
			const XML_Char *tem =
				poolCopyString(&(newDtd->pool), oldE->notation);
			if (!tem)
				return 0;
			newE->notation = tem;
		}
	...
	const char *ptr, const char *end)
{
	if (!pool->ptr && !poolGrow(pool))
		return 0;
	for (;;) {
		XmlConvert(enc, &ptr, end, (ICHAR **)&(pool->ptr),
			(ICHAR *)pool->end);
		if (ptr == end)
			break;
		if (!poolGrow(pool))
...
int poolGrow(STRING_POOL *pool)
{
	...
	if (pool->blocks && pool->start == pool->blocks->s) {
		...
		pool->blocks =
			realloc(pool->blocks, offsetof(BLOCK,
				s) + blockSize * sizeof(XML_Char));
		if (!pool->blocks)
			return 0;
