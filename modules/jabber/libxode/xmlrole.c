...
typedef int PROLOG_HANDLER(struct prolog_state *state,
	int tok, const char *ptr, const char *end, const ENCODING *enc);
	...
	int tok, const char *ptr, const char *end, const ENCODING *enc)
{
	...
	case XML_TOK_DECL_OPEN:
		if (!XmlNameMatchesAscii(enc,
				ptr + 2 * MIN_BYTES_PER_CHAR(enc), "DOCTYPE"))
			break;
	...
	int tok, const char *ptr, const char *end, const ENCODING *enc)
{
	...
	case XML_TOK_DECL_OPEN:
		if (XmlNameMatchesAscii(enc,
				ptr + 2 * MIN_BYTES_PER_CHAR(enc), "ENTITY")) {
			state->handler = entity0;
			return XML_ROLE_NONE;
		}
		if (XmlNameMatchesAscii(enc,
				ptr + 2 * MIN_BYTES_PER_CHAR(enc), "ATTLIST")) {
			state->handler = attlist0;
			return XML_ROLE_NONE;
		}
		if (XmlNameMatchesAscii(enc,
				ptr + 2 * MIN_BYTES_PER_CHAR(enc), "ELEMENT")) {
	...
	case XML_TOK_NAME:
		{
			...
			int i;
			for (i = 0; i < (int)(sizeof(types) / sizeof(types[0]));
				i++)
				if (XmlNameMatchesAscii(enc, ptr, types[i])) {
					state->handler = attlist8;
					return XML_ROLE_ATTRIBUTE_TYPE_CDATA +
						i;
				}
		}
	...
	int tok, const char *ptr, const char *end, const ENCODING *enc)
{
	...
	case XML_TOK_POUND_NAME:
		if (XmlNameMatchesAscii(enc,
				ptr + MIN_BYTES_PER_CHAR(enc), "IMPLIED")) {
			state->handler = attlist1;
			return XML_ROLE_IMPLIED_ATTRIBUTE_VALUE;
		}
		if (XmlNameMatchesAscii(enc,
				ptr + MIN_BYTES_PER_CHAR(enc), "REQUIRED")) {
			state->handler = attlist1;
			return XML_ROLE_REQUIRED_ATTRIBUTE_VALUE;
		}
		if (XmlNameMatchesAscii(enc,
				ptr + MIN_BYTES_PER_CHAR(enc), "FIXED")) {
	...
	int tok, const char *ptr, const char *end, const ENCODING *enc)
{
	...
	case XML_TOK_POUND_NAME:
		if (XmlNameMatchesAscii(enc,
				ptr + MIN_BYTES_PER_CHAR(enc), "PCDATA")) {
