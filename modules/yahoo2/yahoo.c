...
static struct webcam_feed *find_webcam_feed(eb_yahoo_local_account_data *yla,
	const char *who)
{
	YList *l;
...
static eb_account *eb_yahoo_new_account(eb_local_account *ela,
	const char *account);
	...
	for (string_index = 0; msg[string_index]; string_index++) {
		...
		for (color_index = 0; color_index < num_colors; color_index++) {
			if (!strncmp(yahoo_colors[color_index],
				...
				string_index +=
					(strlen(yahoo_colors[color_index]) - 1);
				if (!color_index) {
		...
		for (style_index = 0; style_index < num_styles; style_index++) {
			if (!strncmp(yahoo_styles[style_index],
				...
				string_index +=
					(strlen(yahoo_styles[style_index]) - 1);
				match = 1;
			}
		}
...
static void ext_yahoo_status_changed(int id, const char *who, int stat,
	const char *msg, int away, int idle, int mobile)
{
	...
	if (stat != YAHOO_STATUS_OFFLINE && old_state == YAHOO_STATUS_OFFLINE)
		...
		&& old_state != YAHOO_STATUS_OFFLINE)
		buddy_logoff(ea);
	...
	for (; buds; buds = buds->next) {
		...
		if (ea) {
			...
			if (strcmp(ea->account_contact->nick, contact_name)
				&& !strcmp(ea->account_contact->nick,
					ea->handle))
				rename_contact(ea->account_contact,
					contact_name);
			continue;
		}
...
static void ext_yahoo_got_ignore(int id, YList *ign)
{
	...
	for (; ign; ign = ign->next) {
		...
		if (ea) {
			if (!strcasecmp(ea->account_contact->group->name,
					bud->group))
				continue;
...
static void ext_yahoo_got_im(int id, const char *me, const char *who,
	const char *msg, long tm, int stat, int utf8)
{
	if (stat == 2) {
	...
	if (msg) {
		...
		if (tm) {
			...
			snprintf(newmessage, sizeof(newmessage),
				_
				("<FONT COLOR=\"#0000FF\">[Offline message at %s]</FONT><BR>%s"),
				timestr, msg);
...
static void eb_yahoo_accept_file(gpointer data, int result)
{
	...
	if (result) {
		...
		if (do_prompt_save_file)
			ay_do_file_selection_save(filepath, _("Save file as"),
				eb_yahoo_save_file, yftd);
		else
...
static void eb_yahoo_send_file_callback(AyConnection *fd,
	eb_input_condition cond, gpointer data)
{
	eb_yahoo_file_transfer_data *yftd = data;
	int file = yftd->file;
...
static void eb_yahoo_got_fd(int id, void *fd, int error, void *data)
{
	...
	yftd->input =
		ay_connection_input_add(AY_CONNECTION(fd), EB_INPUT_WRITE,
		eb_yahoo_send_file_callback, yftd);
}

static void eb_yahoo_send_file(eb_local_account *from, eb_account *to,
	char *file)
{
	struct stat stats;
	int in;
	...
	const char *room, const char *msg)
{
	...
	snprintf(buff, sizeof(buff),
		_
		("The yahoo user %s declined your invitation to join conference %s, with the message: %s"),
		who, room, msg);
...
static void ext_yahoo_conf_message(int id, const char *me,
	const char *who, const char *room, const char *msg, int utf8)
{
	int i = 0, j = 0;
...
static void ext_yahoo_chat_userjoin(int id, const char *me, const char *room,
	struct yahoo_chat_member *who)
{
}

static void ext_yahoo_chat_userleave(int id, const char *me, const char *room,
	const char *who)
{
}

static void ext_yahoo_chat_message(int id, const char *me, const char *who,
	const char *room, const char *msg, int msgtype, int utf8)
{
}
	...
	unsigned int real_size, unsigned int timestamp)
{
	...
	if (!wf->image_window_tag) {
		...
		wf->image_window_tag =
			ay_image_window_new(320, 240, buff,
			_image_window_closed, wf);
	}
...
static void ay_yahoo_view_users_webcam(ebmCallbackData *data)
{
	...
	if (!who && ecd->contact && ecd->group) {
		...
		struct contact *c =
			find_contact_in_group_by_nick(ecd->contact, g);
		eb_account *ea =
			find_account_for_protocol(c, SERVICE_INFO.protocol_id);
		who = ea->handle;
	}
	...
	ela = find_local_account_by_handle(ecd->local_account,
		SERVICE_INFO.protocol_id);
	if (!ela) {
		ay_do_warning(_("Yahoo Error"),
			_
			("Cannot find a valid local account to view user's webcam."));
		return;
	}
...
static void ext_yahoo_webcam_invite(int id, const char *me, const char *who)
{
	char buff[1024];
	struct yahoo_authorize_data *wd =
		g_new0(struct yahoo_authorize_data, 1);
	eb_local_account *ela = yahoo_find_local_account_by_id(id);

	snprintf(buff, sizeof(buff),
		_("%s, the yahoo user %s has invited you to view their webcam."
...
static void ext_yahoo_webcam_closed(int id, const char *who, int reason)
{
	...
	snprintf(buff, sizeof(buff), _("%s, webcam connection closed. %s "),
		ela->handle, who);
	switch (reason) {
	case 1:
		strncat(buff, _("stopped broadcasting."),
			sizeof(buff) - strlen(buff));
		break;
	case 2:
		strncat(buff, _("cancelled viewing permission."),
			sizeof(buff) - strlen(buff));
		break;
	case 3:
		strncat(buff, _("declined you permission."),
			sizeof(buff) - strlen(buff));
		break;
	case 4:
		strncat(buff, _("does not have their webcam online."),
			sizeof(buff) - strlen(buff));
		break;
	default:
		strncat(buff, _("did something we don't know about."),
			sizeof(buff) - strlen(buff));
	...
	if ((wf = find_webcam_feed(yla, who))) {
		strncat(buff, _("\nClose image window?"),
			sizeof(buff) - strlen(buff));
		eb_do_dialog(buff, _("Webcam connection closed"),
			eb_yahoo_close_webcam_window, wf);
	} else {
...
static void ay_yahoo_start_webcam(eb_local_account *ela)
{
	...
	if (ay_yahoo_webcam_timeout_callback(wf))
		ylad->webcam_timeout =
			eb_timeout_add(5000,
			(void *)ay_yahoo_webcam_timeout_callback, wf);
}
...
static void ext_yahoo_webcam_viewer(int id, const char *who, int connect)
{
	...
	case 0:
		snprintf(buff, sizeof(buff),
			_
			("%s, the yahoo user %s has stopped viewing your webcam."),
			ela->handle, who);
		yla->viewers--;
		break;
	case 1:
		snprintf(buff, sizeof(buff),
			_("%s, the yahoo user %s is viewing your webcam."),
			ela->handle, who);
		yla->viewers++;
		break;
	case 2:
		snprintf(buff, sizeof(buff),
			_
			("%s, the yahoo user %s wants to view your webcam.\nDo you want to allow this?"),
			ela->handle, who);
		break;
	default:
		snprintf(buff, sizeof(buff),
			_
			("%s, the yahoo user %s wants to %d your webcam.  Tell the ayttm devels about this."),
			ela->handle, who, connect);
		break;
	...
	if (connect != 2) {
		...
		if ((wf = find_webcam_feed(yla, NULL))) {
			snprintf(buff, sizeof(buff),
				_("My webcam (%d viewer%s)"), yla->viewers,
				yla->viewers == 1 ? "" : "s");
			ay_image_window_update_title(wf->image_window_tag,
				buff);
		}
	} else {
		wf = y_new0(struct webcam_feed, 1);
...
static int ay_yahoo_webcam_timeout_callback(gpointer data)
{
	...
	if (!video_grab_frame) {
		...
		ay_do_warning(_("Yahoo Webcam"),
			_
			("Could not read images from your webcam, could not find a video grabber."));
		ay_yahoo_stop_webcam(ela);
		return 0;
	}
	...
	if (wf && image) {
		ay_image_window_add_data(wf->image_window_tag, image, length,
			1);
		ay_image_window_add_data(wf->image_window_tag, NULL, 0, 0);
	}
...
static void ext_yahoo_webcam_invite_reply(int id, const char *me,
	const char *from, int accept)
{
	eb_local_account *ela;
	...
	if (from && !accept) {
		...
		snprintf(buff, sizeof(buff),
			_
			("%s, the yahoo user %s has declined your invitation to view your webcam."),
			ela->handle, from);
...
static void eb_yahoo_authorize_callback(gpointer data, int result)
{
	...
	if (result) {
		...
		yahoo_confirm_buddy(ay->id, ay->who, 1,
...
static void ext_yahoo_rejected(int id, const char *who, const char *msg)
{
	char buff[1024];
	snprintf(buff, sizeof(buff),
		_("%s has rejected your request to be added as a buddy%s%s"),
...
{
	char buff[1024];
	struct yahoo_authorize_data *ay =
		g_new0(struct yahoo_authorize_data, 1);
	eb_local_account *ela = yahoo_find_local_account_by_id(id);
...
static void ext_yahoo_typing_notify(int id, const char *me,
...
static void ext_yahoo_got_search_result(int id, int found,
	int start, int total, YList *contacts)
{
}
...
static void ext_yahoo_mail_notify(int id, const char *from,
	const char *subj, int cnt)
{
	eb_local_account *ela = yahoo_find_local_account_by_id(id);
	eb_yahoo_local_account_data *ylad = ela->protocol_local_account_data;
	char buff[1024] = { 0 }, buff2[200] = {
	0};
...
static void ext_yahoo_system_message(int id, const char *me, const char *who,
	const char *msg)
{
	eb_local_account *ela = yahoo_find_local_account_by_id(id);
	eb_yahoo_local_account_data *ylad = ela->protocol_local_account_data;
...
static void eb_yahoo_finish_login(const char *password, void *data)
{
	...
	ylad->id = yahoo_init_with_attributes(ela->handle, password,
		...
		"conn_type", conn_type, NULL);

	snprintf(buff, sizeof(buff), _("Logging in to Yahoo account: %s"),
		ela->handle);
	ylad->connect_progress_tag =
		ay_activity_bar_add(buff, ay_yahoo_cancel_connect, ela);
...
static void eb_yahoo_login(eb_local_account *ela)
{
	...
	if (ylad->prompt_password || !ylad->password || !strlen(ylad->password)) {
		snprintf(buff, sizeof(buff), _("Yahoo! password for: %s"),
			ela->handle);
...
static void ext_yahoo_login_response(int id, int succ, const char *url)
{
	...
	if (succ == YAHOO_LOGIN_OK) {
		...
		ay_activity_bar_update_label(ylad->connect_progress_tag,
			_("Fetching buddies..."));
		is_setting_state = 1;
		if (ela->status_menu)
		...
		snprintf(buff, sizeof(buff),
			_
			("Could not log into Yahoo service - incorrect password.  Please verify that your password is correctly typed."));
		...
		snprintf(buff, sizeof(buff),
			_
			("Could not log into Yahoo service due to unknown state: %d\n"),
			succ);
	}
...
static int eb_yahoo_get_current_state(eb_local_account *ela)
{
	...
	ylad = ela->protocol_local_account_data;
	if (eb_services[ela->service_id].protocol_id !=
		SERVICE_INFO.protocol_id) {
...
static void eb_yahoo_set_current_state(eb_local_account *ela, int state)
{
	...
	LOG(("ylad->status = %d, state = %d, yahoo_state = %d", ylad->status,
			state, yahoo_state));
	...
	if (yahoo_state == YAHOO_STATUS_CUSTOM) {
		if (ylad->status_message)
			yahoo_set_away(ylad->id, yahoo_state,
				ylad->status_message, ylad->away);
		else
			yahoo_set_away(ylad->id, yahoo_state,
				"delta p * delta x too large", 1);
	} else
...
static void eb_yahoo_change_user_name(eb_account *ea, const char *name)
{
	...
	eb_yahoo_set_buddy_nick(ylad,
		yahoo_find_buddy_by_handle(ylad->id, ea->handle), name);
}
...
/*	LOG(("eb_yahoo_add_user: %s", ea->handle)); */
	...
	for (i = 0; i < 2; i++) {
		if (i == 0)
			buddies = yahoo_get_buddylist(ylad->id);
		else
			buddies = yahoo_get_ignorelist(ylad->id);
	...
	for (i = 0; i < 2; i++) {
		...
		for (; buddy; buddy = buddy->next) {
			...
			if (!strcmp(bud->id, ea->handle)) {
				if (!strcmp(ea->account_contact->group->name,
						_("Ignore")))
					yahoo_ignore_buddy(ylad->id, ea->handle,
						TRUE);
				else
					yahoo_remove_buddy(ylad->id, ea->handle,
						(ea->account_contact ==
							NULL ? "Default" : ea->
							account_contact->group->
							name));
				return;
			}
		}
...
static void eb_yahoo_change_group(eb_account *ea, const char *new_group)
{
	...
	if (!ea->ela->connected
		|| ea->ela->service_id != SERVICE_INFO.protocol_id)
		return;
...
static void eb_yahoo_rename_group(eb_local_account *ela, const char *old_group,
...
{
	yahoo_icon_online =
		gdk_pixbuf_new_from_xpm_data((const char **)yahoo_online_xpm);
	yahoo_icon_away =
		gdk_pixbuf_new_from_xpm_data((const char **)yahoo_away_xpm);
	yahoo_icon_sms =
		gdk_pixbuf_new_from_xpm_data((const char **)yahoo_sms_xpm);
}
...
//              LOG(("eb_yahoo_get_status_string: %s is %s", ea->handle, yad->status_message));
	...
	LOG(("eb_yahoo_get_status_string: %s is Unknown [%d]", ea->handle,
			yad->status));
	return "Unk";
}
...
static void eb_yahoo_set_idle(eb_local_account *ela, int idle)
{
	...
	ylad = ela->protocol_local_account_data;
	if (idle == 0
		&& eb_yahoo_get_current_state(ela) == EB_DISPLAY_YAHOO_IDLE) {
		if (ela->status_menu) {
			eb_set_active_menu_status(ela->status_menu,
				EB_DISPLAY_YAHOO_ONLINE);
		}
	} else if ((idle >= 600)
		&& (eb_yahoo_get_current_state(ela) ==
			EB_DISPLAY_YAHOO_ONLINE)) {
		if (ela->status_menu) {
			eb_set_active_menu_status(ela->status_menu,
				EB_DISPLAY_YAHOO_IDLE);
		}
	}
}
...
static void eb_yahoo_set_away(eb_local_account *ela, char *message, int away)
{
	...
	if (!message) {
		...
		if (do_guess_away) {
			...
			if (strstr(lmsg, "eating") || strstr(lmsg, "breakfast")
				|| strstr(lmsg, "lunch")
				|| strstr(lmsg, "dinner"))
		...
		if (state == EB_DISPLAY_YAHOO_CUSTOM
			&& ylad->status == YAHOO_STATUS_CUSTOM)
			yahoo_set_away(ylad->id, YAHOO_STATUS_CUSTOM,
				ylad->status_message, ylad->away);
...
{
	AyConnection *fd =
		ay_connection_new(host, port,
		use_ssl ? AY_CONNECTION_TYPE_SSL : AY_CONNECTION_TYPE_PLAIN);
	...
	if (use_ssl)
		ccd->tag = ylad->connect_tag =
			ay_connection_connect(fd, _yahoo_connected,
			ay_yahoo_connect_status, eb_do_confirm_dialog, ccd);
	else
		ccd->tag = ylad->connect_tag =
			ay_connection_connect(fd, _yahoo_connected,
			ay_yahoo_connect_status, NULL, ccd);
...
static void eb_yahoo_callback(AyConnection *fd, eb_input_condition condition,
	void *data)
{
	...
	if (condition & EB_INPUT_EXCEPTION)
		LOG(("Exception: %p", fd));
	if (!(condition & (EB_INPUT_READ | EB_INPUT_WRITE |
				EB_INPUT_EXCEPTION)))
...
static int ext_yahoo_add_handler(int id, void *fd, yahoo_input_condition cond,
	void *data)
{
	...
	d->tag = ay_connection_input_add(AY_CONNECTION(fd), eb_cond,
		eb_yahoo_callback, d);
	LOG(("client:%d added fd:%p for cond:%d; tag:%d", id, fd, cond,
			d->tag));
...
static void ext_yahoo_remove_handler(int id, int tag)
{
	...
	for (l = handlers; l; l = l->next) {
		...
		if (d->tag == tag) {
			LOG(("client:%d removed fd:%p with tag:%d", d->id,
					d->fd, d->tag));
...
static void ext_yahoo_got_buddyicon(int id, const char *me, const char *who,
	const char *url, int checksum)
{

}

static void ext_yahoo_got_buddyicon_checksum(int id, const char *me,
	const char *who, int checksum)
{

}

static void ext_yahoo_got_buddyicon_request(int id, const char *me,
	const char *who)
{
...
static void ext_yahoo_got_buddy_change_group(int id, const char *me,
	const char *who, const char *old_group, const char *new_group)
{
...
static char *eb_yahoo_get_color(void)
{
	return "#d08020";
}
...
static void register_menuentries()
{
	mywebcam_chat_menu_tag = eb_add_menu_item(_("Invite to view Webcam"),
		EB_CHAT_WINDOW_MENU,
		ay_yahoo_invite_to_view_my_webcam, ebmCONTACTDATA, NULL);
	...
	webcam_chat_menu_tag = eb_add_menu_item(_("View Webcam"),
		EB_CHAT_WINDOW_MENU,
		ay_yahoo_view_users_webcam, ebmCONTACTDATA, NULL);
...
static void unregister_menuentries()
{
	if (mywebcam_chat_menu_tag)
		eb_remove_menu_item(EB_CHAT_WINDOW_MENU,
			mywebcam_chat_menu_tag);
