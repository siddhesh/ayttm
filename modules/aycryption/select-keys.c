...
update_progress(struct select_keys_s *sk, int running, const char *pattern)
{
	...
	if (!running)
		...
		buf = g_strdup_printf(_("Collecting info for `%s' ... %c"),
			pattern, windmill[running % DIM(windmill)]);
...
/* Release key data */
static gboolean destroy_key(GtkTreeModel *model, GtkTreePath *path,
	GtkTreeIter *iter, gpointer data)
{
	gpgme_key_t key;
	gtk_tree_model_get(model, iter, COL_GPG_KEY, &key, -1);
...
static void destroy_keys(GtkWidget *widget, gpointer data)
{
...
static void set_row(GtkListStore *clist_store, gpgme_key_t key)
{
	...
	while (!temp->subkeys && !finished) {
		...
		if (!gtk_tree_model_iter_next(GTK_TREE_MODEL(clist_store),
				&row))
			finished = TRUE;
		else
			gtk_tree_model_get(GTK_TREE_MODEL(clist_store), &row,
	...
	gtk_list_store_set(clist_store, &newrow,
		...
		COL_VALIDITY, text[COL_VALIDITY], COL_GPG_KEY, key, -1);
...
static void fill_clist(struct select_keys_s *sk, const char *pattern)
{
	...
	g_return_if_fail(sk);
	clist_store =
		GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(sk->
				clist)));
...
static gint cmp_name(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
	gpointer data)
{
	gpgme_key_t pa, pb;
	const char *sa, *sb;

	gtk_tree_model_get(model, a, COL_GPG_KEY, &pa, -1);
	gtk_tree_model_get(model, b, COL_GPG_KEY, &pb, -1);
...
static gint cmp_email(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
	gpointer data)
{
	gpgme_key_t pa, pb;
	const char *sa, *sb;
...
static void create_dialog(struct select_keys_s *sk)
{
	...
	GtkWidget *label;
	GtkWidget *vbox3;
	GtkWidget *select_btn, *cancel_btn, *other_btn, *do_crypt_btn,
		*do_sign_btn;
	...
	g_signal_connect(window, "delete-event", G_CALLBACK(delete_event_cb),
		sk);
	g_signal_connect(window, "key-press-event", G_CALLBACK(key_pressed_cb),
		sk);
	...
	column = gtk_tree_view_column_new_with_attributes(titles[COL_ALGO],
		renderer, "text", COL_ALGO, NULL);
	g_object_set(column, "min-width", 72, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(clist), column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(titles[COL_KEYID],
		renderer, "text", COL_KEYID, NULL);
	g_object_set(column, "min-width", 76, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(clist), column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(titles[COL_NAME],
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(titles[COL_EMAIL],
		renderer, "text", COL_EMAIL, NULL);
	g_object_set(column, "min-width", 130, NULL);
	gtk_tree_view_column_set_sort_column_id(column, COL_EMAIL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(clist), column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(titles[COL_VALIDITY],
		renderer, "text", COL_VALIDITY, NULL);
	...
	select_btn =
		gtkut_create_label_button(_("Select"),
		G_CALLBACK(select_btn_cb), sk);
	cancel_btn =
		gtkut_create_label_button(_("Cancel"),
		G_CALLBACK(cancel_btn_cb), sk);
	other_btn =
		gtkut_create_label_button(_("Other..."),
		G_CALLBACK(other_btn_cb), sk);
	vbox3 = gtk_vbox_new(FALSE, 2);
	do_crypt_btn =
		gtkut_check_button(vbox3, _("Enable encryption"), sk->do_crypt,
		(GCallback) crypt_changed_cb, sk);
	do_sign_btn =
		gtkut_check_button(vbox3, _("Enable signing"), sk->do_sign,
...
static void key_pressed_cb(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
	struct select_keys_s *sk = data;
...
static void select_btn_cb(GtkWidget *widget, gpointer data)
{
	...
	gtk_tree_model_get(model, &selected, COL_GPG_KEY, &key, -1);
...
static void other_selected_cb(const char *text, void *data)
{
	struct select_keys_s *sk = data;
	if (!text)
		return;
...
static void other_btn_cb(GtkWidget *widget, gpointer data)
{
	...
	g_return_if_fail(sk);
	do_text_input_window(_("Enter another user or key ID:"), _(""),
		other_selected_cb, sk);
}
...
static GtkWidget *create_description(const gchar *desc, int prev_was_bad)
{
...
const char *passphrase_mbox(const char *desc, int prev_was_bad)
{
	...
	g_signal_connect(window, "delete-event", G_CALLBACK(passphrase_deleted),
		NULL);
	...
	gtk_button_box_set_layout(GTK_BUTTON_BOX(confirm_box),
		GTK_BUTTONBOX_END);
	...
	g_signal_connect(ok_button, "clicked", G_CALLBACK(passphrase_ok_cb),
		NULL);
	g_signal_connect(pass_entry, "activate", G_CALLBACK(passphrase_ok_cb),
		NULL);
	...
	if (aycrypt_pass_ack) {
		the_passphrase =
			gtk_editable_get_chars(GTK_EDITABLE(pass_entry), 0, -1);
	}
