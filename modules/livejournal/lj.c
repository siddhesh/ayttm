...
static char *ay_lj_get_color(void)
{
	return LJ_MSG_COLOUR;
}
...
static int lj_find_and_handle_errors(int success, eb_local_account *ela,
	LList *pairs)
{
	char buff[1024] = "", *str;
	switch (success) {
	case LJ_HTTP_NETWORK:
		snprintf(buff, sizeof(buff),
			_
			("Could not connect %s to LiveJournal service.\n\nThere was a network error."),
			ela->handle);
		break;
	case LJ_HTTP_NOK:
		snprintf(buff, sizeof(buff),
			_
			("Could not connect %s to LiveJournal service.\n\nThe server returned an unknown HTTP error."),
			ela->handle);
		break;
	case LJ_HTTP_OK:
		...
		if (!str) {
			snprintf(buff, sizeof(buff),
				_
				("Incomplete read from LiveJournal service.\n\nData lost."));
...
static void ay_lj_add_user(eb_account *ea)
{
	...
	snprintf(buff, sizeof(buff),
		"mode=editfriends&user=%s&password=%s&editfriend_add_1_user=%s",
...
static void ay_lj_del_user(eb_account *ea)
{
	...
	snprintf(buff, sizeof(buff),
		"mode=editfriends&user=%s&password=%s&editfriend_delete_%s",
		ela->handle, lla->password, ea->handle);
...
{
	ay_do_info(_("LiveJournal Warning"),
		_("You cannot send files through LiveJournal..."));
}

static void ay_lj_send_im(eb_local_account *account_from,
...
static void _ay_lj_got_update(int success, eb_local_account *ela, LList *pairs)
{
	...
	if (str) {
		if (!strcmp(str, "1")) {
			eb_account *ea =
				find_account_with_ela(ela->handle, ela);
			if (!ea) {
			...
			snprintf(buff, sizeof(buff),
				_
				("%s, your LiveJournal friends view has been updated.<br>"
	...
	if (str) {
		if (lla->poll_interval != atoi(str)) {
			...
			if (lla->poll_timeout_tag) {
				eb_timeout_remove(lla->poll_timeout_tag);
				lla->poll_timeout_tag =
					eb_timeout_add(lla->poll_interval *
					1000, ay_lj_ping_timeout_callback, ela);
			}
		}
		free(str);
	}
...
static int ay_lj_ping_timeout_callback(gpointer data)
{
	...
	snprintf(buff, sizeof(buff),
		"mode=checkfriends&user=%s&password=%s&lastupdate=%s",
...
static void _ay_lj_got_buddies(int success, eb_local_account *ela, LList *pairs)
{
	...
	for (i = 1; i <= nfriends; i++) {
		...
		if (!con) {
			changed = 1;
			con = add_new_contact("LiveJournal", real_name,
...
static void lj_get_buddies(eb_local_account *ela)
{
	...
	snprintf(buff, sizeof(buff), _("Fetching buddies for %s..."),
		ela->handle);
...
static void _ay_lj_got_login(int success, eb_local_account *ela, LList *pairs)
{
	...
	if (lj_find_and_handle_errors(success, ela, pairs)) {
		...
		lj_get_buddies(ela);
		lla->poll_timeout_tag =
			eb_timeout_add(lla->poll_interval * 1000,
			ay_lj_ping_timeout_callback, ela);
	}
}
...
static void ay_lj_login(eb_local_account *ela)
{
	...
	snprintf(buff, sizeof(buff),
		"clientversion=%s-%s/%s&mode=login&user=%s&password=%s", HOST,
		PACKAGE, VERSION, ela->handle, lla->password);
	snprintf(buff2, sizeof(buff2),
		_("Logging in to LiveJournal account: %s"), ela->handle);
...
const char *ext_lj_host()
{
	return (const char *)lj_host;
}
...
const char *ext_lj_path()
{
	return (const char *)lj_path;
}
