...
/*** MIZHI 04162001 */
	gchar *errmsg;
	errmsg = g_new0(gchar,
		128 + strlen(email) + strlen(nick) + strlen(first) +
		strlen(last));
	sprintf(errmsg, "LIBICQ> ICQ_Search(%s, %s, %s, %s)", email, nick,
		first, last);
	ICQ_Debug(ICQ_VERB_INFO, errmsg);
	g_free(errmsg);
/*** */
...
/*** */
	...
	if (c->tcp_sok > 0) {
		open_sockets =
			g_list_remove(open_sockets, (gpointer) c->tcp_sok);
		close(c->tcp_sok);
	}

	if (c->chat_sok > 0) {
		open_sockets =
			g_list_remove(open_sockets, (gpointer) c->chat_sok);
		close(c->chat_sok);
	}
...
void TCP_SendMessages(Contact_Member *c)
{
	...
	while ((msg = g_list_first(c->messages))) {
		...
		c->messages =
			g_list_remove_link(c->messages,
			g_list_first(c->messages));
	}
}
