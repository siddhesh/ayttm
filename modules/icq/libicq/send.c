...
/*** */
			...
			if ((host_struct =
				...
				case NO_RECOVERY:
					errmsg = g_new0(gchar,
						128 + strlen(hostname));
					sprintf(errmsg,
						"Unrecoverable DNS error while looking up %s.\n",
						hostname);
					ICQ_Debug(ICQ_VERB_ERR, errmsg);
					g_free(errmsg);
					return 0;
				case TRY_AGAIN:
					errmsg = g_new0(gchar,
						128 + strlen(hostname));
					sprintf(errmsg,
						"Couldn't look up %s.  Trying again.\n",
						hostname);
					ICQ_Debug(ICQ_VERB_ERR, errmsg);
		...
		while (host_struct->h_addr_list[ip_index]) {
			sin.sin_addr =
				*((struct in_addr *)host_struct->
				h_addr_list[ip_index]);
			errmsg = g_new0(gchar,
				128 + strlen(inet_ntoa(sin.sin_addr)));
			sprintf(errmsg, "Address #%d: %s", ip_index,
	...
	if (retval == -1) {	/* did we connect ? */
		...
		sprintf(errmsg, "Conection Refused on port %d at %s", port,
			host);
		ICQ_Debug(ICQ_VERB_ERR, errmsg);
		g_free(errmsg);
		g_free(host);
	...
	if (retval == -1) {
		if (Verbose & ICQ_VERB_WARN) {
			...
			sprintf(errmsg,
				"Could not bind to tcp socket %d, port %d\n",
				tcp_sok, our_port);
...
void Send_ContactList()
{
	...
	for (i = 0, num_used = 0; i < Num_Contacts; i++) {
		if (Contacts[i].uin > 0
			&& Contacts[i].status != STATUS_NOT_IN_LIST) {
