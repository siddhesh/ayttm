...
int plugin_init()
{
	...
	buddy_list_tag=eb_add_menu_item("GnomeICU Contact List", EB_IMPORT_MENU, import_gnomeicu_accounts, ebmIMPORTDATA, NULL);
	if(!buddy_list_tag)
		return(-1);
	return(0);
}
...
int plugin_finish()
{
	...
	if(result) {
		g_warning("Unable to remove GnomeICU Contact List menu item from import menu!");
		return(-1);
	}
	return(0);
}
