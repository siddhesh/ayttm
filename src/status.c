...
static void create_log_window(void)
{
	...
	gtk_scrolled_window_set_shadow_type(
			GTK_SCROLLED_WINDOW(status_message_swindow), GTK_SHADOW_ETCHED_IN);

	gtk_window_set_title(GTK_WINDOW(status_message_window), _("Ayttm - history"));
	gtk_scrolled_window_set_policy(
			GTK_SCROLLED_WINDOW(status_message_swindow), GTK_POLICY_NEVER, GTK_POLICY_ALWAYS);
	...
	g_signal_connect(
			status_message_window, "delete-event", G_CALLBACK(gtk_widget_hide_on_delete), NULL);
...
static void update_status_message(gchar *message)
{
	...
	if (status_bar) {
		...
		html_text_buffer_append(
				GTK_TEXT_VIEW(status_message_window_text), tstamp_mess, HTML_IGNORE_NONE);
		g_free(tstamp_mess);
	}
}
...
static gboolean collapse_callback(
		GtkTreeView *tree_view, GtkTreeIter *iter, GtkTreePath *path, gpointer data)
{
	gpointer d;
	struct contact *c;
	int type;
...
static gboolean expand_callback(
		GtkTreeView *tree_view, GtkTreeIter *iter, GtkTreePath *path, gpointer data)
{
	gpointer d;
	struct contact *c;
	...
	for (iterator=c->accounts; iterator; iterator = iterator->next) {
		...
		if (account->online && RUN_SERVICE(account)->get_info) {
			buff = g_strdup_printf(
					"%s [%s]", account->handle, get_service_name(account->service_id));
...
static void group_menu(GdkEventButton *event, gpointer d)
{
	...
	gtkut_create_menu_button(GTK_MENU(menu),
			_("Add contact to group"), G_CALLBACK(add_to_group_callback), d);
		...
		struct contact *conn, eb_account *acc, eb_local_account *ela)
{
	...
	for (list = md->menu_items; list; list = list->next) {
		...
		if (mid->protocol) {
			...
			if (!a)
				a = find_suitable_local_account(NULL, get_service_id(mid->protocol));
		...
		||	(mid->protocol != NULL
			...
			&& find_account_for_protocol(
						conn ? conn : acc->account_contact, get_service_id(mid->protocol)))
		) {
			if (last != cur_service) {
				if (should_sep)
				...
				if (!acc && mid->protocol) {
					...
					gtkut_attach_submenu(GTK_MENU(menu), title, submenu, ecd->local_account != NULL);
			...
			eb_debug(
					DBG_CORE, "ecd->contact:%s, ecd->ra=%s, ecd->la=%s\n",
			...
			g_signal_connect(button, "activate", G_CALLBACK(eb_generic_menu_function), mid);
			gtk_widget_show(button);
			last = cur_service;
...
static void contact_menu(GdkEventButton *event, gpointer d)
{
	...
	gtkut_create_menu_button(GTK_MENU(menu),
			_("Add Account to Contact"), G_CALLBACK(add_account_to_contact_callback), d);

	gtkut_create_menu_button(GTK_MENU(menu),
			_("Edit Contact"), G_CALLBACK(edit_contact_callback), d);

	gtkut_create_menu_button(GTK_MENU(menu),
			_("Delete Contact"), G_CALLBACK(offer_remove_contact_callback), d);
	...
	if ((md = GetPref(EB_CONTACT_MENU))) {
		int should_sep = 0;
		for (cur_service = -1;
			cur_service < 0 || strcmp(get_service_name(cur_service), "unknown");
			cur_service++)
...
static void account_menu(GdkEventButton *event, gpointer d)
{
	...
	gtkut_create_menu_button(GTK_MENU(menu),
			_("Delete Account..."), G_CALLBACK(offer_remove_account_callback), d);

	gtkut_create_menu_button(GTK_MENU(menu), NULL, NULL, NULL);
			/* sep */

	if (CAN(acc, send_file)) gtkut_create_menu_button(GTK_MENU(menu),
...
static gboolean right_click(GtkWidget *widget, GdkEventButton *event, gpointer d)
{
	...
	if (version_cmp(version, PACKAGE_VERSION "-" RELEASE) > 0) {
		...
		if (warn_again || !warned || strcmp(warned, version)) {
			char *buf = g_strdup_printf(
					_("A new release of ayttm is available.\n"
...
void update_group_line(grouplist * eg)
{
	...
	gtk_tree_store_set(
			contact_list_store, eg->list_item,
			MAIN_VIEW_LABEL, eg->name,
			MAIN_VIEW_ROW_DATA, eg,
			-1);
}
...
void add_contact_line(struct contact *ec)
{
	...
	gtk_tree_store_insert_before(
			contact_list_store, &iter, (GtkTreeIter *)ec->group->list_item, sibling);
	...
	ec->expanded ? MAIN_VIEW_EXPAND_ROW(ec->list_item) : MAIN_VIEW_COLLAPSE_ROW(ec->list_item);
}
...
void remove_account_line(eb_account *ea)
{
	...
	if (ea->account_contact->list_item)
		gtk_tree_store_remove(GTK_TREE_STORE(contact_list_store), ea->list_item);
...
static gint update_window_title(struct contact *ec)
{
	...
	if (ec) {
		title = g_strdup_printf(
				"(%c %s)", ec->online ? '+' : '-', ec->nick);
		window_title_handler = eb_timeout_add(
				5000, (GtkFunction)update_window_title, NULL);
	}
	else
...
void buddy_logoff(eb_account *ea)
{
	...
	ea->icon_handler = eb_timeout_add(5000, (GtkFunction)hide_account, (gpointer) ea);
}
...
static GtkWidget *MakeContactList()
{
	...
	column = gtk_tree_view_column_new_with_attributes(
			NULL, renderer,	"pixbuf", MAIN_VIEW_ICON, NULL);
	...
	gtk_tree_sortable_set_sort_column_id(
			GTK_TREE_SORTABLE(contact_list_store), 1, GTK_SORT_ASCENDING);
	...
	gtk_drag_dest_set(
			contact_list, GTK_DEST_DEFAULT_ALL,
			drag_types, 1,
			GDK_ACTION_DEFAULT|GDK_ACTION_MOVE);
	...
	gtk_scrolled_window_set_policy(
			GTK_SCROLLED_WINDOW(contact_window), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
...
static GtkWidget *MakeStatusMenu(eb_local_account *ela)
{
	...
	for (temp_list = status_label, x = 0; temp_list; x++, temp_list = temp_list->next) {
		...
		gtk_menu_shell_append(GTK_MENU_SHELL(status_menu), status_menu_item);
		g_signal_connect(
				status_menu_item, "activate", G_CALLBACK(eb_status), (gpointer) stats);
		g_signal_connect(
				status_menu_item, "remove", G_CALLBACK(eb_status_remove), (gpointer) stats);
	}
...
void eb_profile_window(void *v_profile_submenuitem)
{
	...
	if ((md = GetPref(EB_PROFILE_MENU))) {
		for (list = md->menu_items; list; list = list->next) {
			...
			gtk_menu_shell_append(GTK_MENU_SHELL(profile_menu), label);
			g_signal_connect(
					label, "activate", G_CALLBACK(eb_profile_function), mid);
			gtk_widget_show(label);
			added = TRUE;
		}
	}
...
void eb_smiley_window(void *v_smiley_submenuitem)
{
	...
	if (md) {
		...
		for (--i; i >= 0; i--)
			g_signal_connect(
					items[i].label, "activate", G_CALLBACK(eb_smiley_function), items[i].mid);
		free(items);
	}
...
void eb_import_window(void *v_import_submenuitem)
{
	...
	if ((md = GetPref(EB_IMPORT_MENU))) {
		for (list = md->menu_items; list; list = list->next) {
			...
			gtk_menu_shell_append(GTK_MENU_SHELL(import_menu), label);
			g_signal_connect(
					label, "activate", G_CALLBACK(eb_import_function), mid);
			gtk_widget_show(label);
		}
	}
...
void set_menu_sensitivity(void)
{
	...
	menu_set_sensitive(ui_manager, "ui/menubar/Chat/SignonAll", (online != l_list_length(accounts)));
...
static void get_main_menu(GtkWidget *window, GtkWidget **menubar, GtkWidget *menubox)
{
	GError *error = NULL;
...
//	g_free(userrc);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(label), status_show == 2);
	gtk_widget_show(label);
	radioonline = label;
	...
	gtk_box_pack_start(GTK_BOX(hbox), label, TRUE, TRUE, 1);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(label), status_show == 0);
	gtk_widget_show(label);
	radioaccount = label;
	...
	g_signal_connect(radioonline, "clicked", G_CALLBACK(status_show_callback), GINT_TO_POINTER(2));
	g_signal_connect(radiocontact, "clicked", G_CALLBACK(status_show_callback), GINT_TO_POINTER(1));
	g_signal_connect(radioaccount, "clicked", G_CALLBACK(status_show_callback), GINT_TO_POINTER(0));
