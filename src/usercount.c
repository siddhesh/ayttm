...
void show_wnd_usercount(void)
{
	...
	gtk_widget_set_usize(label1, 410, 0);
	...
	gtk_widget_set_usize(label2, 410, 0);
	...
	gtk_button_box_set_child_size(GTK_BUTTON_BOX(hbuttonbox1), 70, 0);

	btn_no = gtk_button_new_with_label("");
	btn_no_key = gtk_label_parse_uline(GTK_LABEL(GTK_BIN(btn_no)->child), "_No");
	...
	btn_yes = gtk_button_new_with_label("");
	btn_yes_key = gtk_label_parse_uline(GTK_LABEL(GTK_BIN(btn_yes)->child), "_Yes");
	...
	gtk_signal_connect_object_after(GTK_OBJECT(btn_no), "clicked",
					GTK_SIGNAL_FUNC
					(gtk_widget_destroy),
					GTK_OBJECT(wnd_usercount));
	gtk_signal_connect(GTK_OBJECT(btn_no), "clicked",
			   GTK_SIGNAL_FUNC(ay_usercount),
			   GINT_TO_POINTER(0));
	gtk_signal_connect_object_after(GTK_OBJECT(btn_yes), "clicked",
					GTK_SIGNAL_FUNC
					(gtk_widget_destroy),
					GTK_OBJECT(wnd_usercount));
	...
	gtk_signal_connect(GTK_OBJECT(wnd_usercount), "destroy",
