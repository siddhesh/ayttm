...
static char * get_fqdn(const char *host)
{
	struct hostent *hp;
...
SSLCertificate *ssl_certificate_new(X509 *x509_cert, char *host, int port)
{
...
int ssl_certificate_check (X509 *x509_cert, char *host, int port)
{
	...
	if (known_cert == NULL) {
		...
		eb_do_no_callback_dialog( err_msg, "SSL certificate warning", &result);
