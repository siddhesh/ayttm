...
enum {
	CHAT_ROOM_NOTIFICATION_JOIN = 0x777777,
	CHAT_ROOM_NOTIFICATION_LEAVE = 0x777777,
	CHAT_ROOM_NOTIFICATION_NICK_CHANGE = 0x777777
};

struct _chat_window;
typedef struct _chat_window eb_chat_room;

#define eb_chat_room_display_notification(ecr, msg,type) ({ \
	eb_chat_window_display_notification((chat_window *)ecr, msg, type); \
})
...
extern "C" {
	...
	eb_chat_room *eb_start_chat_room(eb_local_account *ela, char *name,
		int is_public);
	void eb_chat_room_buddy_arrive(eb_chat_room *room, const char *alias,
		const char *handle);
	void eb_chat_room_buddy_chnick(eb_chat_room *room, const char *buddy,
		const char *newnick);
	void eb_chat_room_buddy_leave(eb_chat_room *room, const char *handle);
	void eb_chat_room_buddy_leave_ex(eb_chat_room *room, const char *handle,
		const char *ex);
	void eb_chat_room_refresh_list(eb_chat_room *room, const char *handle,
