...
static void delete_data_cb(GtkWidget *menuitem, gpointer d)
{
	GtkTreeSelection *selection =
		gtk_tree_view_get_selection(GTK_TREE_VIEW(data_clist));
...
void set_popup_menu_position(GtkMenu *menu, gint *x, gint *y, gboolean *push_in,
	gpointer data)
{
	...
	if (event) {
		...
		*y = event->y_root;
		...
		path = gtk_tree_model_get_path(GTK_TREE_MODEL(data_clist_model),
			&selected);

		gtk_tree_view_get_cell_area(GTK_TREE_VIEW(data_clist), path,
			NULL, &rectangle);
		gtk_tree_view_get_visible_rect(GTK_TREE_VIEW(data_clist),
			&visible_rect);

		*x = wx + data_clist->allocation.x + visible_rect.x +
			visible_rect.width / 2;
		*y = wy + data_clist->allocation.y + rectangle.y +
			rectangle.height / 2;
	}
}
...
void do_popup_menu(GtkTreeView *tree_view, GdkEventButton *event)
{
	...
	if (event) {
		...
		g_signal_handlers_block_by_func(tree_view,
			G_CALLBACK(select_data_cb), NULL);
	...
	g_signal_connect(menubutton, "activate", G_CALLBACK(delete_data_cb),
		NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menubutton);
	gtk_widget_show(menubutton);

	gtk_menu_popup(GTK_MENU(menu), NULL, NULL, set_popup_menu_position,
		event, button, event_time);
}
...
static void build_data_clist()
{
	...
	g_signal_connect(selection, "changed", G_CALLBACK(select_data_cb),
		NULL);
	g_signal_connect(data_clist, "row-activated",
		G_CALLBACK(clicked_data_cb), NULL);
	g_signal_connect(data_clist, "button-press-event",
		G_CALLBACK(right_click_cb), NULL);
	g_signal_connect(data_clist, "popup-menu", G_CALLBACK(popup_menu_cb),
		NULL);
...
static void save_data(void)
{
	...
	if (!buff || strlen(buff) == 0) {
		snprintf(mbuf, 1024, _("You have to fill in \"%s\"."),
			datatype);
	...
	char *valuename, void (*cb) (char *msg, void *data), void *cbdata)
{
	if (!data_open) {
		...
		g_signal_connect(title, "changed", G_CALLBACK(check_title),
			NULL);
		...
		if (help) {
			...
			gtk_widget_show(label);
			gtk_box_pack_start(GTK_BOX(vbox2), label, TRUE, TRUE,
				0);
		}
		...
		gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW
			(scrollwindow), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
		...
		g_signal_connect(button, "clicked", G_CALLBACK(ok_callback),
			NULL);
		...
		g_signal_connect(button, "clicked", G_CALLBACK(cancel_callback),
			NULL);
