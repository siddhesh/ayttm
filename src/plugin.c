...
static int compare_plugin_loaded_service(gconstpointer a, gconstpointer b)
{
	...
	if (epi->service && epi->status == PLUGIN_LOADED
		&& !strcmp(epi->service, (char *)b))
		return (0);
	return (1);
}
	...
	gboolean force)
{
	...
	if (status == PLUGIN_CANNOT_LOAD) {
		...
		snprintf(buff, 512,
			_
			("Plugin %s has not been loaded because of an error:\n\n%s"),
			name, status_desc);
		ay_do_error(_("Plugin error"), buff);
	}
}

/* Find names which end in .la for libtool and .so for dlopen, the expected module extension */
	...
	do {
		if ((dirp = opendir(cur_path)) == NULL) {
			snprintf(buf, sizeof(buf),
				_("Cannot open module directory \"%s\"."),
				cur_path);
		...
		if (n == 0) {
			eb_debug(DBG_CORE,
				"<No modules found in %s, returning.\n",
				cur_path);
		} else {
...
#endif
{
	...
	if (epi && epi->status == PLUGIN_LOADED) {
		fprintf(stderr,
			_
			("Not loading module %s, a module for that service is already loaded!\n"),
			name);
		SetPluginInfo(info, name, NULL, PLUGIN_CANNOT_LOAD,
			_
	...
	if (!module_version || module_version() != CORE_VERSION) {
		...
		if (!module_version) {
			strncpy(error,
				_("This plugin is not binary compatible.\n"
					"Try a clean install (or read README)."),
				error_len);
		} else {
			snprintf(error, error_len,
				_
				("Plugin version (%d) differs from core version (%d), "
	...
	SetPluginInfo(info, name, Module, PLUGIN_LOADED, "", Service_Info->name,
		TRUE);
	eb_debug(DBG_CORE, "Added module:%s, service: %s\n", name,
		eb_services[service_id].name);
	eb_debug(DBG_CORE,
		"eb_services[%i].sc->read_local_account_config: %p\n",
		service_id,
		eb_services[service_id].sc->read_local_account_config);
	return (0);
}
	...
	const char *name)
{
	...
	if (!info->init) {
		...
		snprintf(buf, buf_len,
			_
			("Utility module %s doesn't define an init function; skipping it."),
			name);
		ay_do_warning(_("Plugin Warning"), buf);
...
int unload_module_full_path(const char *inFullPath)
{
	...
	if (epi == NULL) {
		eb_debug(DBG_CORE,
			"<Plugin not found: %s [file: %s line: %d]\n",
...
int unload_module(eb_PLUGIN_INFO *epi)
{
	...
	if (epi->pi.finish) {
		...
		if (error > 0) {
			snprintf(buf, sizeof(buf),
				_
				("Unable to unload plugin %s; maybe it is still in use?"),
				epi->name);
...
static void rebuild_smiley_menu(void)
{
	...
	if (!smiley_submenuitem) {
		eb_debug(DBG_CORE,
			"Not rebuilding smiley menu, it's never been built.\n");
		return;
	}
...
static void rebuild_import_menu(void)
{
	...
	if (!import_submenuitem) {
		eb_debug(DBG_CORE,
			"Not rebuilding import menu, it's never been built.\n");
		return;
	}
...
static void rebuild_profile_menu(void)
{
	...
	if (!profile_submenuitem) {
		eb_debug(DBG_CORE,
			"Not rebuilding profile menu, it's never been built.\n");
		return;
	}
