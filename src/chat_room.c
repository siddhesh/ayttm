...
static gboolean handle_focus(GtkWidget *widget, GdkEventFocus *event,
	gpointer userdata);
static void eb_chat_room_update_window_title(eb_chat_room *ecb,
...
static void handle_fellow_click(char *name, eb_chat_room *cr)
{
	...
	if (find_account_by_handle(name, cr->local_user->service_id))
		...
		&& find_account_by_handle(handle, cr->local_user->service_id))
		...
		ea = RUN_SERVICE(cr->local_user)->new_account(cr->local_user,
			name);
	...
	GtkTreeViewColumn *column, gpointer data)
{
	...
	gtk_tree_model_get_iter(GTK_TREE_MODEL(ecr->fellows_model), &iter,
		path);
	gtk_tree_model_get(GTK_TREE_MODEL(ecr->fellows_model), &iter, 0, &name,
		-1);
...
static void send_cr_message(GtkWidget *widget, gpointer data)
{
	...
	GtkTextBuffer *buffer =
		gtk_text_view_get_buffer(GTK_TEXT_VIEW(ecr->entry));
...
static gboolean cr_key_press(GtkWidget *widget, GdkEventKey *event,
	gpointer data)
{
	...
	if (event->keyval == GDK_Return) {
		...
		/* ... otherwise simply print out the grub */
			...
			gtk_text_buffer_delete_selection
				(gtk_text_view_get_buffer(GTK_TEXT_VIEW(cr->
						entry)), FALSE, TRUE);
		...
		&& check_tab_accelerators(widget, cr, modifiers, event))
	...
	if (do_complete) {
		if (event->keyval == GDK_space || ispunct(event->keyval)) {
			...
			complete_mode = TRUE;
			if (!chat_auto_complete(cr->entry,
					auto_complete_session_words, event))
				return chat_auto_complete(cr->entry,
					cr->fellows, event);
...
static void set_sound_on_toggle(GtkWidget *sound_button, gpointer userdata)
{
	...
	g_signal_handlers_block_by_func(cr->sound_button, set_sound_on_toggle,
		userdata);
	gtk_toggle_tool_button_set_active(GTK_TOGGLE_TOOL_BUTTON(cr->
			sound_button), cr->sound_enabled);
	g_signal_handlers_unblock_by_func(cr->sound_button, set_sound_on_toggle,
		userdata);
...
static void eb_add_auto_chatroom(eb_local_account *ela, const char *name,
	int public)
{
	FILE *fp;
	char buff[4096];
...
static void eb_remove_auto_chatroom(eb_local_account *ela, const char *name,
	int public)
{
	FILE *fp, *tmp;
	...
	gpointer userdata)
{
	...
	if (gtk_toggle_tool_button_get_active(GTK_TOGGLE_TOOL_BUTTON
			(reconnect_button)))
		eb_add_auto_chatroom(cr->local_user, cr->room_name,
			cr->is_public);
	else
		eb_remove_auto_chatroom(cr->local_user, cr->room_name,
			cr->is_public);
}
...
static void invite_callback(GtkWidget *widget, gpointer data)
{
	...
	char *acc = NULL;
	char *invited =
		gtk_combo_box_get_active_text(GTK_COMBO_BOX(ecr->invite_buddy));
	...
	if (ecr->preferred) {
		...
		*strstr(acc, ")") = '\0';
		...
		if (third) {
			chat_window_to_chat_room(ecr, third,
				gtk_entry_get_text(GTK_ENTRY(ecr->
						invite_message)));
		...
		acc = strstr(invited, "(") + 1;
		*strstr(acc, ")") = '\0';
		RUN_SERVICE(ecr->local_user)->send_invite(ecr->local_user, ecr,
			acc,
...
void do_invite_window(void *widget, eb_chat_room *room)
{
	...
	if (!room->local_user) {
		ay_do_error(_("Chatroom error"),
			_
			("Cannot invite a third party until a protocol has been chosen."));
		return;
	}

	room->invite_window = gtk_window_new(GTK_WINDOW_TOPLEVEL);

	gtk_window_set_transient_for(GTK_WINDOW(room->invite_window),
		GTK_WINDOW(room->window));
	gtk_window_set_position(GTK_WINDOW(room->invite_window),
		GTK_WIN_POS_MOUSE);
	gtk_window_set_title(GTK_WINDOW(room->invite_window),
		_("Invite a contact"));
	...
	gtk_table_attach(GTK_TABLE(table), mbox, 0, 1, 0, 1, GTK_FILL, GTK_FILL,
		0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), room->invite_buddy, 1, 2, 0, 1,
	...
	gtk_widget_show(mbox);
	gtk_box_pack_end(GTK_BOX(mbox), label, FALSE, FALSE, 5);
	gtk_table_attach(GTK_TABLE(table), mbox, 0, 1, 1, 2, GTK_FILL, GTK_FILL,
		0, 0);
	gtk_widget_show(label);

	room->invite_message = gtk_entry_new();
	gtk_table_attach(GTK_TABLE(table), room->invite_message, 1, 2, 1, 2,
		GTK_FILL, GTK_FILL, 0, 0);
	...
	g_signal_connect_swapped(label, "clicked",
		G_CALLBACK(gtk_widget_destroy), room->invite_window);
	mbox = gtk_hbox_new(FALSE, 0);
	...
	g_signal_connect(room->invite_window, "destroy",
		G_CALLBACK(destroy_invite), room);
}
...
static void join_chat_callback(GtkWidget *widget, gpointer data)
{
	...
	*(strstr(service, "]")) = '\0';
	...
	eb_start_chat_room(ela, name,
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(public_chkbtn)));
...
{
	GtkWidget *label, *frame, *table, *vbox, *hbox, *hbox2, *button,
		*separator;
	...
	gtk_window_set_transient_for(GTK_WINDOW(join_chat_window),
	...
	public_list_btn =
		gtk_button_new_with_label(_("List public chatrooms..."));
	...
	g_signal_connect(button, "clicked", G_CALLBACK(join_chat_callback),
		NULL);
	...
	g_signal_connect_swapped(button, "clicked",
		G_CALLBACK(gtk_widget_destroy), join_chat_window);
	gtk_box_pack_start(GTK_BOX(hbox2), button, TRUE, TRUE, 0);
	gtk_widget_show(button);
	...
	g_signal_connect(join_chat_window, "destroy",
		G_CALLBACK(join_chat_destroy), NULL);
}
...
static void destroy(GtkWidget *widget, gpointer data)
{
	...
	g_signal_handlers_disconnect_by_func(ecr->window,
		G_CALLBACK(handle_focus), ecr);
...
void eb_chat_room_refresh_list(eb_chat_room *room, const char *buddy,
	ChatRoomRefreshType refresh)
{
	GtkTreeIter insert;
	gchar *buf;
	...
	if (refresh == CHAT_ROOM_JOIN) {
		...
		if (!gtk_tree_model_get_iter_first(GTK_TREE_MODEL(room->
		...
		do {
			...
			gtk_tree_model_get(GTK_TREE_MODEL(room->fellows_model),
				&del_iter, 0, &name, -1);
			...
			if (buddy && name && !strcmp(name, buddy)) {
				...
				buf = g_strdup_printf(_("Online: %i"),
					room->total_arrivals);
				gtk_tree_view_column_set_title(room->column,
					buf);
				g_free(buf);
				gtk_list_store_remove(room->fellows_model,
					&del_iter);
				return;
			}
		}
		while (gtk_tree_model_iter_next(GTK_TREE_MODEL(room->
					fellows_model), &del_iter));
	}
}
...
static void eb_chat_room_private_log_reference(eb_chat_room *room,
	const char *alias, const char *handle)
{
	...
	if (err) {
		eb_debug(DBG_CORE,
			"eb_chat_room_private_log_reference: could not open log file [%s]\n",
			buff);
	...
	g_snprintf(buff, buff_size,
		"You had a <a href=\"log://%s\">group chat with %s (%s)</a>.\n",
...
void eb_chat_room_buddy_arrive(eb_chat_room *room, const gchar *alias,
	const gchar *handle)
{
	...
	for (t = room->fellows; t && t->data; t = t->next)
		if (!strcasecmp(handle,
				((eb_chat_room_buddy *)t->data)->handle))
			return;
...
void eb_chat_room_buddy_chnick(eb_chat_room *room, const gchar *buddy,
	const gchar *newnick)
{
	GtkTreeIter iter;
	gchar *buf;
	...
	if (!gtk_tree_model_get_iter_first(GTK_TREE_MODEL(room->fellows_model),
			&iter)) {
	...
	do {
		char *name = NULL;
		gtk_tree_model_get(GTK_TREE_MODEL(room->fellows_model), &iter,
			0, &name, -1);

		if (name && !strcmp(name, buddy)) {
			gtk_list_store_set(room->fellows_model, &iter, 0,
				newnick, -1);
			return;
		}
	}
	while (gtk_tree_model_iter_next(GTK_TREE_MODEL(room->fellows_model),
			&iter));
}
	...
	gpointer userdata)
{
	...
	if (cr->notebook
		&& cr->notebook_page !=
		gtk_notebook_get_current_page(GTK_NOTEBOOK(cr->notebook)))
		return FALSE;
...
void eb_chat_room_buddy_leave_ex(eb_chat_room *room, const gchar *handle,
	const gchar *ex)
{
	...
	if (node && room->fellows) {
		eb_chat_room_buddy *ecrb = node->data;
		eb_account *ea =
			find_account_by_handle(ecrb->handle,
	...
	gboolean new_message)
{
	...
	room_title = g_strdup_printf("%s%s [%s]",
		new_message ? "* " : "",
		ecb->room_name, get_service_name(ecb->local_user->service_id));
	gtk_window_set_title(GTK_WINDOW(ecb->window), room_title);
	g_free(room_title);
}

static eb_chat_room *find_chatroom_by_ela_and_name(eb_local_account *ela,
...
eb_chat_room *eb_start_chat_room(eb_local_account *ela, gchar *name,
	int is_public)
{
	eb_chat_room *ecb = NULL;

	eb_debug(DBG_CORE, "Starting chatroom %s(%s)\n", name,
		(is_public ? "public" : "non-public"));
	...
	const gchar *message)
{
	...
	if (!strcmp(chat_room->local_user->handle, user)) {
		...
		for (walk = chat_room->typing_fellows; walk && walk->data;
			...
			if (!strcasecmp(acc->handle, user)
				|| !strcasecmp(acc->account_contact->nick,
					user))
				break;
			else
				acc = NULL;
	...
	html_text_buffer_append(GTK_TEXT_VIEW(chat_room->chat), buff,
		HTML_IGNORE_NONE);
	...
	gpointer userdata)
{
	...
	if (event->type == GDK_BUTTON_PRESS && event->button == 3) {
		...
		/* Sound Selection */
		button = gtk_menu_item_new_with_label(_(cr->
				sound_enabled ? "Disable Sounds" :
				"Enable Sounds"));

		g_signal_connect(button, "activate",
			G_CALLBACK(set_sound_on_toggle), cr);
		...
		if (cr->encoding)
			snprintf(encoding_label, sizeof(encoding_label),
				_("Set Character Encoding (%s)"), cr->encoding);
		else
			snprintf(encoding_label, sizeof(encoding_label),
		...
		g_signal_connect(button, "activate",
			G_CALLBACK(ay_set_chat_encoding), cr);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), button);
		gtk_widget_show(button);
		...
		g_signal_connect(button, "activate",
			G_CALLBACK(do_invite_window), cr);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), button);
		gtk_widget_show(button);
		...
		g_signal_connect(button, "activate",
			G_CALLBACK(destroy_chat_room), cr);
...
void eb_join_chat_room(eb_chat_room *chat_room, int send_join)
{
	...
	chat_room->fellows_widget =
		gtk_tree_view_new_with_model(GTK_TREE_MODEL(chat_room->
			fellows_model));
	...
	chat_room->column =
		...
		"text", 0, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(chat_room->fellows_widget),
		chat_room->column);

	gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(chat_room->
			fellows_model), 0, GTK_SORT_ASCENDING);
	...
	g_signal_connect(chat_room->chat, "button-press-event",
		G_CALLBACK(cr_handle_click), chat_room);
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrollwindow2),
		GTK_SHADOW_ETCHED_IN);
	...
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(chat_room->entry),
		GTK_WRAP_WORD_CHAR);
	...
	g_signal_connect(chat_room->entry, "key-press-event",
		G_CALLBACK(cr_key_press), chat_room);
	...
	g_signal_connect(label, "clicked", G_CALLBACK(invite_button_callback),
		chat_room);
	...
	gtk_paned_pack2(GTK_PANED(hbox), chat_room->fellows_widget, FALSE,
		FALSE);
	...
	g_signal_connect(chat_room->window, "focus-in-event",
		G_CALLBACK(handle_focus), chat_room);
...
#define ICON_CREATE_XPM(icon,iconwid,xpm) {\
	...
	TOOLBAR_APPEND_TOGGLE_BUTTON(chat_room->reconnect_button,
		...
		_("Reconnect at login"),
		iconwid, set_reconnect_on_toggle, chat_room);

	gtk_toggle_tool_button_set_active(GTK_TOGGLE_TOOL_BUTTON(chat_room->
			reconnect_button), eb_is_chatroom_auto(chat_room));

	ICON_CREATE_XPM(icon, iconwid, tb_volume_xpm);
	TOOLBAR_APPEND_TOGGLE_BUTTON(chat_room->sound_button,
		_("Sound"),
		_("Enable Sounds"), iconwid, set_sound_on_toggle, chat_room);
	...
	gtk_toggle_tool_button_set_active(GTK_TOGGLE_TOOL_BUTTON(chat_room->
			sound_button), enableSoundButton);
	...
	TOOLBAR_APPEND(send_button, _("Send Message"), iconwid, send_cr_message,
		chat_room);
...
#undef TOOLBAR_APPEND_SPACE
	...
	if (RUN_SERVICE(chat_room->local_user)->join_chat_room == NULL) {
		eb_debug(DBG_CORE,
			"!RUN_SERVICE(chat_room->local_user)->join_chat_room\n");
		return;
	}
	...
	if (chat_room->notebook)
		gtk_notebook_set_current_page(GTK_NOTEBOOK(chat_room->notebook),
			chat_room->notebook_page);
}
...
static void get_group_contacts(gchar *group, eb_chat_room *room)
{
	...
	while (node) {
		...
		while (accounts) {
			if (((struct account *)accounts->data)->ela ==
				room->local_user
				&& ((struct account *)accounts->data)->online) {
				char *buf =
					g_strdup_printf("%s (%s)",
					contact->nick,
					((struct account *)accounts->data)->
					handle);
				gtk_combo_box_append_text(GTK_COMBO_BOX(room->
						invite_buddy), buf);
			}
	...
	for (; rooms && rooms->data; rooms = rooms->next) {
		...
		if (message && !strcmp(_("typing..."), message)) {
			if (!ecr->typing_fellows
				...
				ecr->typing_fellows =
					l_list_append(ecr->typing_fellows,
					remote);
		...
		for (walk2 = ecr->typing_fellows; walk2 && walk2->data;
			walk2 = walk2->next) {
