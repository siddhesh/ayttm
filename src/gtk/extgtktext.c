...
static void  gtk_text_class_init     (ExtGtkTextClass   *klass);
...
static void  gtk_text_get_arg        (GtkObject      *object,
				      GtkArg         *arg,
				      guint           arg_id);
static void  gtk_text_init           (ExtGtkText        *text);
...
static void ext_gtk_text_insert_text       (GtkEditable       *editable,
...
static void move_gap (ExtGtkText* text, guint index);
static void make_forward_space (ExtGtkText* text, guint len);

/* Property management */
static ExtGtkTextFont* get_text_font (VFont* gfont);
...
static void init_properties      (ExtGtkText *text);
static void realize_property     (ExtGtkText *text, TextProperty *prop);
static void realize_properties   (ExtGtkText *text);
static void unrealize_property   (ExtGtkText *text, TextProperty *prop);
static void unrealize_properties (ExtGtkText *text);

static void delete_text_property (ExtGtkText* text, guint len);

static guint pixel_height_of (ExtGtkText* text, GList* cache_line);

/* Property Movement and Size Computations */
static void advance_mark (ExtGtkPropertyMark* mark);
static void decrement_mark (ExtGtkPropertyMark* mark);
static void advance_mark_n (ExtGtkPropertyMark* mark, gint n);
static void decrement_mark_n (ExtGtkPropertyMark* mark, gint n);
static void move_mark_n (ExtGtkPropertyMark* mark, gint n);
static ExtGtkPropertyMark find_mark (ExtGtkText* text, guint mark_position);
static ExtGtkPropertyMark find_mark_near (ExtGtkText* text, guint mark_position, const ExtGtkPropertyMark* near);
static void find_line_containing_point (ExtGtkText* text, guint point,
					gboolean scroll);

/* Display */
static void compute_lines_pixels (ExtGtkText* text, guint char_count,
				  guint *lines, guint *pixels);

static gint total_line_height (ExtGtkText* text,
			       GList* line,
			       gint line_count);
static LineParams find_line_params (ExtGtkText* text,
				    const ExtGtkPropertyMark *mark,
				    const PrevTabCont *tab_cont,
				    PrevTabCont *next_cont);
static void recompute_geometry (ExtGtkText* text);
static void insert_expose (ExtGtkText* text, guint old_pixels, gint nchars, guint new_line_count);
static void delete_expose (ExtGtkText* text,
			   guint nchars,
			   guint old_lines, 
			   guint old_pixels);
static GdkGC *create_bg_gc (ExtGtkText *text);
static void clear_area (ExtGtkText *text, GdkRectangle *area);
static void draw_line (ExtGtkText* text,
		       gint pixel_height,
		       LineParams* lp);
static void draw_cursor (ExtGtkText* text, gint absolute);
static void undraw_cursor (ExtGtkText* text, gint absolute);
static gint drawn_cursor_min (ExtGtkText* text);
static gint drawn_cursor_max (ExtGtkText* text);
static void expose_text (ExtGtkText* text, GdkRectangle *area, gboolean cursor);

/* Search and Placement. */
static void find_cursor (ExtGtkText* text,
			 gboolean scroll);
static void find_cursor_at_line (ExtGtkText* text,
				 const LineParams* start_line,
				 gint pixel_height);
static void find_mouse_cursor (ExtGtkText* text, gint x, gint y);

/* Scrolling. */
static void adjust_adj  (ExtGtkText* text, GtkAdjustment* adj);
static void scroll_up   (ExtGtkText* text, gint diff);
static void scroll_down (ExtGtkText* text, gint diff);
static void scroll_int  (ExtGtkText* text, gint diff);

static void process_exposes (ExtGtkText *text);

/* Cache Management. */
static void   free_cache        (ExtGtkText* text);
static GList* remove_cache_line (ExtGtkText* text, GList* list);

/* Key Motion. */
static void move_cursor_buffer_ver (ExtGtkText *text, int dir);
static void move_cursor_page_ver (ExtGtkText *text, int dir);
static void move_cursor_ver (ExtGtkText *text, int count);
static void move_cursor_hor (ExtGtkText *text, int count);
...
/* To be removed */
static void gtk_text_move_forward_character    (ExtGtkText          *text);
static void gtk_text_move_backward_character   (ExtGtkText          *text);
static void gtk_text_move_forward_word         (ExtGtkText          *text);
static void gtk_text_move_backward_word        (ExtGtkText          *text);
static void gtk_text_move_beginning_of_line    (ExtGtkText          *text);
static void gtk_text_move_end_of_line          (ExtGtkText          *text);
static void gtk_text_move_next_line            (ExtGtkText          *text);
static void gtk_text_move_previous_line        (ExtGtkText          *text);

static void gtk_text_delete_forward_character  (ExtGtkText          *text);
static void gtk_text_delete_backward_character (ExtGtkText          *text);
static void gtk_text_delete_forward_word       (ExtGtkText          *text);
static void gtk_text_delete_backward_word      (ExtGtkText          *text);
static void gtk_text_delete_line               (ExtGtkText          *text);
static void gtk_text_delete_to_line_end        (ExtGtkText          *text);
static void gtk_text_select_word               (ExtGtkText          *text,
						guint32           time);
static void gtk_text_select_line               (ExtGtkText          *text,
						guint32           time);

static void gtk_text_set_position  (GtkEditable       *editable,
				    gint               position);

/* #define DEBUG_EXT_GTK_TEXT */

#if defined(DEBUG_EXT_GTK_TEXT) && defined(__GNUC__)
/* Debugging utilities. */
static void gtk_text_assert_mark (ExtGtkText         *text,
				  ExtGtkPropertyMark *mark,
				  ExtGtkPropertyMark *before,
				  ExtGtkPropertyMark *after,
				  const gchar     *msg,
				  const gchar     *where,
				  gint             line);

static void gtk_text_assert (ExtGtkText         *text,
			     const gchar     *msg,
			     gint             line);
static void gtk_text_show_cache_line (ExtGtkText *text, GList *cache,
				      const char* what, const char* func, gint line);
static void gtk_text_show_cache (ExtGtkText *text, const char* func, gint line);
static void gtk_text_show_adj (ExtGtkText *text,
			       GtkAdjustment *adj,
			       const char* what,
			       const char* func,
			       gint line);
static void gtk_text_show_props (ExtGtkText* test,
				 const char* func,
				 int line);
...
#ifdef HAVE_LIBXFT
static XftFont * default_font = NULL;
#endif
...
#ifdef HAVE_LIBXFT
void xft_draw_text     (GdkDrawable  *drawable,
		        XftFont      *font,
			GdkGC        *gc,
			GdkColor     *color,
			gint          x,
			...
			gint          text_length)
{
	...
	if(!use_wchar)
	{
		XftDrawString8(xftdraw, &xftcolor, font, x, y, (unsigned char *) text,text_length);
	}
	else
	{
		XftDrawString32(xftdraw, &xftcolor, font, x, y, (unsigned char *) text,text_length);
	}
...
static void
gtk_text_class_init (ExtGtkTextClass *class)
{
  ...
  gtk_object_add_arg_type ("ExtGtkText::hadjustment",
			   GTK_TYPE_ADJUSTMENT,
			   GTK_ARG_READWRITE | GTK_ARG_CONSTRUCT,
			   ARG_HADJUSTMENT);
  gtk_object_add_arg_type ("ExtGtkText::vadjustment",
			   GTK_TYPE_ADJUSTMENT,
			   GTK_ARG_READWRITE | GTK_ARG_CONSTRUCT,
			   ARG_VADJUSTMENT);
  gtk_object_add_arg_type ("ExtGtkText::line_wrap",
			   GTK_TYPE_BOOL,
			   GTK_ARG_READWRITE,
			   ARG_LINE_WRAP);
  gtk_object_add_arg_type ("ExtGtkText::word_wrap",
			   GTK_TYPE_BOOL,
			   GTK_ARG_READWRITE,
			   ARG_WORD_WRAP);
  ...
  widget_class->set_scroll_adjustments_signal =
    gtk_signal_new ("set_scroll_adjustments",
		    GTK_RUN_LAST,
		    object_class->type,
		    GTK_SIGNAL_OFFSET (ExtGtkTextClass, set_scroll_adjustments),
		    gtk_marshal_NONE__POINTER_POINTER,
		    GTK_TYPE_NONE, 2, GTK_TYPE_ADJUSTMENT, GTK_TYPE_ADJUSTMENT);

  editable_class->set_editable = gtk_text_real_set_editable;
  editable_class->insert_text = ext_gtk_text_insert_text;
  ...
  class->set_scroll_adjustments = ext_gtk_text_set_adjustments;
}
		  ...
		  guint             arg_id)
{
  ...
  text = EXT_GTK_TEXT (object);
  
  switch (arg_id)
    {
    case ARG_HADJUSTMENT:
      ext_gtk_text_set_adjustments (text,
    ...
    case ARG_VADJUSTMENT:
      ...
      break;
    case ARG_LINE_WRAP:
      ext_gtk_text_set_line_wrap (text, GTK_VALUE_BOOL (*arg));
      break;
    case ARG_WORD_WRAP:
      ext_gtk_text_set_word_wrap (text, GTK_VALUE_BOOL (*arg));
      break;
    default:
      break;
    }
...
GtkWidget*
ext_gtk_text_new (GtkAdjustment *hadj,
	      GtkAdjustment *vadj)
{
  GtkWidget *text;
#ifdef HAVE_LIBXFT
  if(!default_font)
  {
	  default_font =  XftFontOpen(gdk_display, DefaultScreen(gdk_display),XFT_FAMILY, XftTypeString,
			                                      "Helvetica",XFT_FAMILY, XftTypeString,
			                                      "Arial",XFT_SIZE, XftTypeInteger, 10,0);
  }
#endif
  ...
  text = gtk_widget_new (EXT_GTK_TYPE_TEXT,
			 "hadjustment", hadj,
			 "vadjustment", vadj,
			 NULL);
...
void
ext_gtk_text_set_word_wrap (ExtGtkText *text,
			gint     word_wrap)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
...
void
ext_gtk_text_set_line_wrap (ExtGtkText *text,
			gint     line_wrap)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
...
void
ext_gtk_text_set_editable (ExtGtkText *text,
		       gboolean is_editable)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
			    ...
			    gboolean     is_editable)
{
  ...
  g_return_if_fail (EXT_GTK_IS_TEXT (editable));
  
  text = EXT_GTK_TEXT (editable);
...
void
ext_gtk_text_set_adjustments (ExtGtkText       *text,
			  GtkAdjustment *hadj,
			  GtkAdjustment *vadj)
{
  g_return_if_fail (text != NULL);
...
void
ext_gtk_text_set_point (ExtGtkText *text,
		    guint    index)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
...
guint
ext_gtk_text_get_point (ExtGtkText *text)
{
  g_return_val_if_fail (text != NULL, 0);
  g_return_val_if_fail (EXT_GTK_IS_TEXT (text), 0);
  
  return text->point.index;
}

guint
ext_gtk_text_get_length (ExtGtkText *text)
{
  g_return_val_if_fail (text != NULL, 0);
  g_return_val_if_fail (EXT_GTK_IS_TEXT (text), 0);
...
void
ext_gtk_text_freeze (ExtGtkText *text)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
...
void
ext_gtk_text_thaw (ExtGtkText *text)
{
  g_return_if_fail (text != NULL);
  g_return_if_fail (EXT_GTK_IS_TEXT (text));
			  ...
			  gint        fill)
{
  ...
  if (text->use_wchar)
    ...
    {
      numwcs = length;
      memcpy(text->text.ch + text->gap_position, chars, strlen(chars));
    }
...
/*      insert_text_property (text, font, fore, back, numwcs); */
  ...
  if (frozen)
    ext_gtk_text_thaw (text);
}

void
ext_gtk_text_insert (ExtGtkText    *text,
                 VFont*font,
	         GdkColor   *fore,
	         GdkColor   *back,
                 const char *chars,
...
void
ext_gtk_text_insert_underlined (ExtGtkText    *text,
                            VFont*font,
	                    GdkColor   *fore,
	                    GdkColor   *back,
                            const char *chars,
...
void
ext_gtk_text_insert_pixmap  (ExtGtkText *text,
                            VFont*font,
	                    GdkColor   *fore,
	                    GdkColor   *back,
                            GdkDrawable *image,
...
void
ext_gtk_text_insert_data_underlined   (ExtGtkText *text,
                                        VFont*font,
                                        GdkColor   *fore,
                                        GdkColor   *back,
...
void
ext_gtk_text_insert_data   (ExtGtkText *text,
                            VFont*font,
	                    GdkColor   *fore,
	                    GdkColor   *back,
                            gpointer user_data_n,
...
gint
ext_gtk_text_backward_delete (ExtGtkText *text,
			  guint    nchars)
{
  g_return_val_if_fail (text != NULL, 0);
  g_return_val_if_fail (EXT_GTK_IS_TEXT (text), 0);
  ...
  ext_gtk_text_set_point (text, text->point.index - nchars);
  
  return ext_gtk_text_forward_delete (text, nchars);
}

gint
ext_gtk_text_forward_delete (ExtGtkText *text,
			 guint    nchars)
{
  ...
  if (text->point.index < text->first_line_start_index)
      ...
      if (text->point.index + nchars >= text->first_line_start_index)
	{
	  ...
	  while ((text->first_line_start_index > 0) &&
		 (EXT_GTK_TEXT_INDEX (text, text->first_line_start_index - 1)
		  != LINE_DELIM))
		    ...
		    gint           end_pos)
{
  ...
  g_return_val_if_fail (EXT_GTK_IS_TEXT (editable), NULL);
  text = EXT_GTK_TEXT (editable);
...
gtk_text_destroy (GtkObject *object)
{
  ...
  g_return_if_fail (EXT_GTK_IS_TEXT (object));
  
  text = (ExtGtkText*) object;
...
gtk_text_realize (GtkWidget *widget)
{
  ...
  g_return_if_fail (EXT_GTK_IS_TEXT (widget));
  
  text = EXT_GTK_TEXT (widget);
...
gtk_text_style_set (GtkWidget *widget,
		    GtkStyle  *previous_style)
{
  ExtGtkText *text = EXT_GTK_TEXT (widget);
...
#else
  text->current_font = get_text_font(default_font);
#endif
}
...
static void
clear_focus_area (ExtGtkText *text, gint area_x, gint area_y, gint area_width, gint area_height)
{
	       ...
	       GdkRectangle *area)
{
  ...
  if (GTK_WIDGET_DRAWABLE (widget))
    {
      expose_text (EXT_GTK_TEXT (widget), area, TRUE);
		 ...
		 GdkEventExpose *event)
{
  ...
  g_return_val_if_fail (EXT_GTK_IS_TEXT (widget), FALSE);
  g_return_val_if_fail (event != NULL, FALSE);
  
  if (event->window == EXT_GTK_TEXT (widget)->text_area)
    {
      TDEBUG (("in gtk_text_expose (expose)\n"));
      expose_text (EXT_GTK_TEXT (widget), &event->area, TRUE);
    }
			...
			GdkEventMotion *event)
{
  ...
  undraw_cursor (EXT_GTK_TEXT (widget), FALSE);
  find_mouse_cursor (EXT_GTK_TEXT (widget), x, y);
  draw_cursor (EXT_GTK_TEXT (widget), FALSE);
...
static void 
ext_gtk_text_insert_text    (GtkEditable       *editable,
			 const gchar       *new_text,
			 gint               new_text_length,
			 gint              *position)
{
  ExtGtkText *text = EXT_GTK_TEXT (editable);
  VFont*font;
  GdkColor *fore, *back;
  gboolean underlined;
  GdkDrawable *image;
  ...
  guint user_data_length=0;
  DataFunc *user_data_func=NULL;
  TextProperty *property;

  ext_gtk_text_set_point (text, *position);
  ...
  underlined = property->flags & PROPERTY_UNDERLINED ? TRUE : FALSE;
  image = property->flags & PROPERTY_IMAGE ? property->image : NULL; 
  mask = property->flags & PROPERTY_IMAGE ? property->mask: NULL; 
  if (property->flags & PROPERTY_DATA)
  {

      user_data_length=property->user_data_length;
      user_data  = g_malloc(user_data_length);
			 ...
			 gint               end_pos)
{
  ...
  ext_gtk_text_set_point (text, start_pos);
  if (end_pos < 0)
    end_pos = TEXT_LENGTH (text);
  
  if (end_pos > start_pos)
    ext_gtk_text_forward_delete (text, end_pos - start_pos);
}
...
#ifdef USE_XIM
  if (GTK_EDITABLE(widget)->ic)
    gdk_im_begin (GTK_EDITABLE(widget)->ic, EXT_GTK_TEXT(widget)->text_area);
#endif
  
  draw_cursor (EXT_GTK_TEXT(widget), TRUE);
		    ...
		    GdkEventFocus *event)
{
  ...
  undraw_cursor (EXT_GTK_TEXT(widget), TRUE);
		     ...
		     ExtGtkText       *text)
{
  ...
  if (adjustment == text->hadj)
    ext_gtk_text_set_adjustments (text, NULL, text->vadj);
  if (adjustment == text->vadj)
    ext_gtk_text_set_adjustments (text, text->hadj, NULL);
}


static ExtGtkPropertyMark
find_this_line_start_mark (ExtGtkText* text, guint point_position, const ExtGtkPropertyMark* near)
{
  ExtGtkPropertyMark mark;
  ...
  while (mark.index > 0 &&
	 EXT_GTK_TEXT_INDEX (text, mark.index - 1) != LINE_DELIM)
    decrement_mark (&mark);
...
static void
init_tab_cont (ExtGtkText* text, PrevTabCont* tab_cont)
{
...
static void
line_params_iterate (ExtGtkText* text,
...
static gint
fetch_lines_iterator (ExtGtkText* text, LineParams* lp, void* data)
{
...
static void
fetch_lines_backward (ExtGtkText* text)
{
...
static void
fetch_lines_forward (ExtGtkText* text, gint line_count)
{
  ExtGtkPropertyMark mark;
 ...
 */
static void
compute_lines_pixels (ExtGtkText* text, guint char_count,
		      guint *lines, guint *pixels)
{
...
static gint
total_line_height (ExtGtkText* text, GList* line, gint line_count)
{
  gint height = 0;
...
static void
swap_lines (ExtGtkText* text, GList* old, GList* new, guint old_line_count)
{
...
static void
correct_cache_delete (ExtGtkText* text, gint nchars, gint lines)
{
  ...
  for (; cache; cache = cache->next)
    {
      ExtGtkPropertyMark *start = &CACHE_DATA(cache).start;
      ExtGtkPropertyMark *end = &CACHE_DATA(cache).end;
...
static void
delete_expose (ExtGtkText* text, guint nchars, guint old_lines, guint old_pixels)
{
  GtkWidget *widget = GTK_WIDGET (text);
...
static void
correct_cache_insert (ExtGtkText* text, gint nchars)
{
  GList *cache;
  ExtGtkPropertyMark *start;
  ExtGtkPropertyMark *end;
...
static void
insert_expose (ExtGtkText* text, guint old_pixels, gint nchars,
...
{
#ifndef HAVE_LIBXFT
  return gdk_font_id ((const VFont*) font);
#else
  return (guint)font;
#endif
}
...
gboolean xft_font_equal(XftFont * fonta, XftFont * fontb)
{
	if(fonta->ascent != fontb->ascent)
	{
		return FALSE;
	}
	if(fonta->descent != fontb->descent)
	{
		return FALSE;
	}
	if(fonta->height != fontb->height)
	{
		return FALSE;
	}
	if(fonta->max_advance_width != fontb->max_advance_width)
	{
		return FALSE;
	}
	return TRUE;
...
static ExtGtkTextFont*
get_text_font (VFont* gfont)
{
  ExtGtkTextFont* tf=NULL;
  gint i;
...
#else
  if (!font_cache_table)
    font_cache_table = g_hash_table_new (font_hash, (GCompareFunc) xft_font_equal);
#endif
  ...
  {
	  XGlyphInfo extents;
	  char string[] = {(char)i, '\0'};
	  XftTextExtents8(gdk_display, gfont, string, 1, &extents);
	  tf->char_widths[i] = extents.xOff;
  }
#endif
...
static void
text_font_unref (ExtGtkTextFont *text_font)
{
  text_font->ref_count--;
                        ...
                        DataFunc *user_data_func)
{
  ...
  if (((prop->flags & PROPERTY_UNDERLINED)?TRUE:FALSE)!=underlined)
    return FALSE;
  ...
  if (prop->flags & PROPERTY_DATA)
  {
       if  (memcmp(prop->user_data, user_data,user_data_length)!=0)
        return FALSE;
  }
  else 
    if (user_data!=NULL)
...
static void
realize_property (ExtGtkText *text, TextProperty *prop)
{
...
static void
realize_properties (ExtGtkText *text)
{
...
static void
unrealize_property (ExtGtkText *text, TextProperty *prop)
{
  GdkColormap *colormap = gtk_widget_get_colormap (GTK_WIDGET (text));
...
static void
unrealize_properties (ExtGtkText *text)
{
  GList *tmp_list = text->text_properties;
		   ...
		   DataFunc *user_data_func, guint length)
{
  ...
  if (underlined)
    {
      prop->flags |= PROPERTY_UNDERLINED;
    }
  ...
  if (user_data)
    {
      prop->flags |= PROPERTY_DATA;
      prop->user_data_length=user_data_length;
      prop->user_data=g_malloc(user_data_length);
      prop->user_data_func=user_data_func;
      memcpy(prop->user_data, user_data, user_data_length);
    }
  else 
    {
        prop->user_data=NULL;
        prop->user_data_length=0;
        prop->user_data_func=NULL;
    }
...
destroy_text_property (TextProperty *prop)
{
  ...
  if (prop->user_data)
    g_free(prop->user_data);  
  if (prop->image)
    gdk_pixmap_unref(prop->image);
  if (prop->mask)
    gdk_pixmap_unref(prop->mask);
 ...
 * dead fish. */
static void
move_gap (ExtGtkText* text, guint index)
{
...
static void
make_forward_space (ExtGtkText* text, guint len)
{
...
insert_text_property (ExtGtkText* text, VFont* font,
		      ...
		      GdkDrawable* image, GdkBitmap *mask, 
                      ...
                      guint len)
{
  ExtGtkPropertyMark *mark = &text->point;
  ...
  if (MARK_OFFSET(mark) == 0)
	       ...
	       (forward_prop->length == 1))
	{
	  ...
	  if (underlined)
	    {
	      forward_prop->flags |= PROPERTY_UNDERLINED;
	    }
          if (image)
            {
              forward_prop->flags |= PROPERTY_IMAGE;
              forward_prop->image = image;
	      forward_prop->mask = mask;
            }
	  if(divider)
	  {
          ...
          if (user_data!=NULL) 
            {
              ...
              forward_prop->user_data_length=user_data_length;
              forward_prop->user_data = g_malloc(user_data_length);  
              memcpy(forward_prop->user_data , user_data, user_data_length);   
              forward_prop->user_data_func=user_data_func;
            }
          else
            {
              forward_prop->user_data=NULL;
              forward_prop->user_data_length=0;
...
static void
delete_text_property (ExtGtkText* text, guint nchars)
{
...
static void
init_properties (ExtGtkText *text)
{
...
static void
move_mark_n (ExtGtkPropertyMark* mark, gint n)
{
  if (n > 0)
...
static void
advance_mark (ExtGtkPropertyMark* mark)
{
...
static void
advance_mark_n (ExtGtkPropertyMark* mark, gint n)
{
  gint i;
  TextProperty* prop;
...
static void
decrement_mark (ExtGtkPropertyMark* mark)
{
  mark->index -= 1;
...
static void
decrement_mark_n (ExtGtkPropertyMark* mark, gint n)
{
  g_assert (n > 0);
...
static ExtGtkPropertyMark
find_mark (ExtGtkText* text, guint mark_position)
{
 ...
 */
static ExtGtkPropertyMark
find_mark_near (ExtGtkText* text, guint mark_position, const ExtGtkPropertyMark* near)
{
  gint diffa;
  gint diffb;
...
static void
find_line_containing_point (ExtGtkText* text, guint point,
			    gboolean scroll)
{
  GList* cache;
  gint height;
...
static guint
pixel_height_of (ExtGtkText* text, GList* cache_line)
{
...
static gint
find_char_width (ExtGtkText* text, const ExtGtkPropertyMark *mark, const TabStopMark *tab_mark)
{
  GdkWChar ch;
  gint16* char_widths;
  GdkDrawable* image;
  ...
  ch = EXT_GTK_TEXT_INDEX (text, mark->index);
  char_widths = MARK_CURRENT_TEXT_FONT (text, mark)->char_widths;
  if((image=MARK_CURRENT_IMAGE(text, mark)))
  {
#ifdef __MINGW32__
     return ((GdkDrawablePrivate*) image) ->width + 2;
#else
     return ((GdkWindowPrivate*) image) ->width + 2;
#endif
  }
...
static void
advance_tab_mark (ExtGtkText* text, TabStopMark* tab_mark, GdkWChar ch)
{
...
static void
advance_tab_mark_n (ExtGtkText* text, TabStopMark* tab_mark, gint n)
     /* No tabs! */
{
  while (n--)
...
static void
find_cursor_at_line (ExtGtkText* text, const LineParams* start_line, gint pixel_height)
{
  GdkWChar ch;
#ifdef USE_XIM
  ...
  while (mark.index < text->cursor_mark.index)
    {
      ...
      advance_tab_mark (text, &tab_mark, EXT_GTK_TEXT_INDEX(text, mark.index));
      advance_mark (&mark);
    }
  ...
  ch = LAST_INDEX (text, mark) ? 
    LINE_DELIM : EXT_GTK_TEXT_INDEX (text, mark.index);
...
static void
find_cursor (ExtGtkText* text, gboolean scroll)
{
  if (GTK_WIDGET_REALIZED (text))
    {
...
static void
find_mouse_cursor_at_line (ExtGtkText *text, const LineParams* lp,
			   guint line_pixel_height,
			   gint button_x)
{
  ExtGtkPropertyMark mark     = lp->start;
...
static void
find_mouse_cursor (ExtGtkText* text, gint x, gint y)
{
  gint pixel_height;
  GList* cache = text->line_start_cache;
...
static GList*
remove_cache_line (ExtGtkText* text, GList* member)
{
  GList *list;
...
static void
move_cursor_buffer_ver (ExtGtkText *text, int dir)
{
  undraw_cursor (text, FALSE);
...
static void
move_cursor_page_ver (ExtGtkText *text, int dir)
{
  scroll_int (text, dir * text->vadj->page_increment);
}

static void
move_cursor_ver (ExtGtkText *text, int count)
{
  gint i;
  ExtGtkPropertyMark mark;
  gint offset;
  ...
  if (count < 0)
  ...
  else
    {
      ...
      while (!LAST_INDEX(text, mark) && EXT_GTK_TEXT_INDEX(text, mark.index) != LINE_DELIM)
	advance_mark (&mark);
  ...
  for (i=0; i < text->cursor_virtual_x; i += 1, advance_mark(&mark))
    if (LAST_INDEX(text, mark) ||
	EXT_GTK_TEXT_INDEX(text, mark.index) == LINE_DELIM)
      break;
...
static void
move_cursor_hor (ExtGtkText *text, int count)
{
  /* count should be +-1. */
		      ...
		      gint         y)
{
  if (x > 0)
    {
      while (x-- != 0)
	move_cursor_hor (EXT_GTK_TEXT (editable), 1);
    }
  else if (x < 0)
    {
      while (x++ != 0)
  ...
  if (y > 0)
    {
      while (y-- != 0)
	move_cursor_ver (EXT_GTK_TEXT (editable), 1);
    }
  else if (y < 0)
    {
      while (y++ != 0)
...
static void
gtk_text_move_forward_character (ExtGtkText *text)
{
  move_cursor_hor (text, 1);
}

static void
gtk_text_move_backward_character (ExtGtkText *text)
{
  move_cursor_hor (text, -1);
}

static void
gtk_text_move_next_line (ExtGtkText *text)
{
  move_cursor_ver (text, 1);
}

static void
gtk_text_move_previous_line (ExtGtkText *text)
{
  move_cursor_ver (text, -1);
}
		    ...
		    gint         n)
{
  if (n > 0)
    {
      while (n-- != 0)
	gtk_text_move_forward_word (EXT_GTK_TEXT (editable));
    }
  else if (n < 0)
    {
      while (n++ != 0)
	gtk_text_move_backward_word (EXT_GTK_TEXT (editable));
    }
}

static void
gtk_text_move_forward_word (ExtGtkText *text)
{
  text->cursor_virtual_x = 0;
  ...
  if (text->use_wchar)
    {
      while (!LAST_INDEX (text, text->cursor_mark) && 
	     !gdk_iswalnum (EXT_GTK_TEXT_INDEX(text, text->cursor_mark.index)))
    ...
    {
      while (!LAST_INDEX (text, text->cursor_mark) && 
	     !isalnum (EXT_GTK_TEXT_INDEX(text, text->cursor_mark.index)))
	advance_mark (&text->cursor_mark);
...
static void
gtk_text_move_backward_word (ExtGtkText *text)
{
  text->cursor_virtual_x = 0;
  ...
  if (text->use_wchar)
    {
      while ((text->cursor_mark.index > 0) &&
	     !gdk_iswalnum (EXT_GTK_TEXT_INDEX(text, text->cursor_mark.index-1)))
    ...
    {
      while ((text->cursor_mark.index > 0) &&
	     !isalnum (EXT_GTK_TEXT_INDEX(text, text->cursor_mark.index-1)))
	decrement_mark (&text->cursor_mark);
...
{
  if (y != 0)
    scroll_int (EXT_GTK_TEXT (editable), 
		y * EXT_GTK_TEXT(editable)->vadj->page_increment);  
}
			 ...
			 gint         column)
{
  ...
  while ((text->cursor_mark.index > 0) &&
	 (EXT_GTK_TEXT_INDEX (text, text->cursor_mark.index - 1) != LINE_DELIM))
    decrement_mark (&text->cursor_mark);
  
  while (!LAST_INDEX (text, text->cursor_mark) &&
	 (EXT_GTK_TEXT_INDEX (text, text->cursor_mark.index) != LINE_DELIM))
    {
      if (column > 0)
	column--;
      else if (column == 0)
...
static void
gtk_text_move_beginning_of_line (ExtGtkText *text)
{
...
static void
gtk_text_move_end_of_line (ExtGtkText *text)
{
...
static void
gtk_text_delete_forward_character (ExtGtkText *text)
{
  gtk_text_kill_char (GTK_EDITABLE (text), 1);
}

static void
gtk_text_delete_backward_character (ExtGtkText *text)
{
  gtk_text_kill_char (GTK_EDITABLE (text), -1);
}
...
static void
gtk_text_delete_forward_word (ExtGtkText *text)
{
  gtk_text_kill_word (GTK_EDITABLE (text), 1);
}

static void
gtk_text_delete_backward_word (ExtGtkText *text)
{
  gtk_text_kill_word (GTK_EDITABLE (text), -1);
}
...
static void
gtk_text_delete_line (ExtGtkText *text)
{
  gtk_text_move_to_column (GTK_EDITABLE (text), 0);
...
static void
gtk_text_delete_to_line_end (ExtGtkText *text)
{
  gtk_text_kill_line (GTK_EDITABLE (text), 1);
}

static void
gtk_text_select_word (ExtGtkText *text, guint32 time)
{
  gint start_pos;
  gint end_pos;
...
static void
gtk_text_select_line (ExtGtkText *text, guint32 time)
{
  gint start_pos;
  gint end_pos;
...
static void
adjust_adj (ExtGtkText* text, GtkAdjustment* adj)
{
  gint height;
...
static gint
set_vertical_scroll_iterator (ExtGtkText* text, LineParams* lp, void* data)
{
...
static gint
set_vertical_scroll_find_iterator (ExtGtkText* text, LineParams* lp, void* data)
{
  SetVerticalScrollData *svdata = (SetVerticalScrollData *) data;
  gint return_val;
...
static ExtGtkPropertyMark
set_vertical_scroll (ExtGtkText* text)
{
  ExtGtkPropertyMark mark = find_mark (text, 0);
...
static void
scroll_int (ExtGtkText* text, gint diff)
{
  gfloat upper;
...
static void 
process_exposes (ExtGtkText *text)
{
  GdkEvent *event;
...
static gint last_visible_line_height (ExtGtkText* text)
{
  GList *cache = text->line_start_cache;
  gint height;
...
static gint first_visible_line_height (ExtGtkText* text)
{
...
static void
scroll_down (ExtGtkText* text, gint diff0)
{
  GdkRectangle rect;
  gint real_diff = 0;
  gint width, height;
...
static void
scroll_up (ExtGtkText* text, gint diff0)
{
  gint real_diff = 0;
  GdkRectangle rect;
  gint width, height;
...
static LineParams
find_line_params (ExtGtkText* text,
		  const ExtGtkPropertyMark* mark,
		  const PrevTabCont *tab_cont,
		  PrevTabCont *next_cont)
{
  ...
  while (!LAST_INDEX(text, lp.end))
    {
      ...
      ch   = EXT_GTK_TEXT_INDEX (text, lp.end.index);
      ...
      if (ch == LINE_DELIM)
	{
	  ...
	  if (!lp.font_ascent && !lp.font_descent)
	    {
	      lp.font_ascent = font->ascent+1;
	      lp.font_descent = font->descent+1;
	    }
	  ...
	  if (text->line_wrap)
	    {
	      ...
	      /*
	       * if it is an image that is overflowing,
	       * always wrap it
	       */

	      if(MARK_CURRENT_IMAGE(text, &lp.end))
	      {
	      ...
	      else
		{
		  if (text->word_wrap)
		    {
		      ExtGtkPropertyMark saved_mark = lp.end;
		      ...
		      if (text->use_wchar)
			{
			  while (!gdk_iswspace (EXT_GTK_TEXT_INDEX (text, lp.end.index)) &&
				 (lp.end.index > lp.start.index))
			    {
			      decrement_mark (&lp.end);
			      lp.displayable_chars -= 1;
			...
			{
			  while (!isspace(EXT_GTK_TEXT_INDEX (text, lp.end.index)) &&
				 (lp.end.index > lp.start.index))
			    {
			      decrement_mark (&lp.end);
			      lp.displayable_chars -= 1;
      ...
      else
	{
	  lp.displayable_chars += 1;
        ...
        if((image=MARK_CURRENT_IMAGE(text, &lp.end)))
        {
#ifdef __MINGW32__
             int height=((GdkDrawablePrivate*) image) ->height;
	     int pixel_width =((GdkDrawablePrivate*) image) ->width+2;
#else
             int height=((GdkWindowPrivate*) image) ->height;
	     int pixel_width =((GdkWindowPrivate*) image) ->width+2;
#endif
	     lp.pixel_width += pixel_width;
	     lp.font_ascent = MAX(((height*3)/4)+1, lp.font_ascent);
	     lp.font_descent = MAX((height/4)+1, lp.font_descent);
	}
	else
	{
      		lp.font_ascent = MAX (font->ascent+1, lp.font_ascent);
      		lp.font_descent = MAX (font->descent+1, lp.font_descent);
...
static void
expand_scratch_buffer (ExtGtkText* text, guint len)
{
...
static void
draw_bg_rect (ExtGtkText* text, ExtGtkPropertyMark *mark,
	   ...
	   LineParams* lp)
{
  ...
  if (mark.index <= text->gap_position &&
	...
	{
	  for (i = 0; i < chars; i += 1)
	    text->scratch_buffer.wc[i] = EXT_GTK_TEXT_INDEX(text, mark.index + i);
          buffer.wc = text->scratch_buffer.wc;
	}
      else
	{
	  for (i = 0; i < chars; i += 1)
	    text->scratch_buffer.ch[i] = EXT_GTK_TEXT_INDEX(text, mark.index + i);
	  ...
	  (!text->use_wchar && buffer.ch[0] != '\t'))
	{
	  ...
	  if (font->type == GDK_FONT_FONT)
		...
		pixel_width = gdk_text_width (font, buffer.ch, len);
	      ...
	      XGlyphInfo glyph;
	      if(text->use_wchar)
	      	XftTextExtents32(gdk_display, font, buffer.ch, len, &glyph);
	      else
	      	XftTextExtents8(gdk_display, font, buffer.ch, len, &glyph);
	      pixel_width = glyph.xOff;
#endif
	    }
          if((image=MARK_CURRENT_IMAGE(text, &mark)))
	  {
#ifdef __MINGW32__
	     pixel_width =((GdkDrawablePrivate*) image) ->width +2;
#else
	  ...
	  if ((mark.index >= selection_start_pos) && 
		...
		fg_color = &GTK_WIDGET(text)->style->fg[GTK_STATE_SELECTED];
	      }
	      else
	      {
		fg_gc = GTK_WIDGET(text)->style->fg_gc[GTK_STATE_ACTIVE];
		fg_color = &GTK_WIDGET(text)->style->fg[GTK_STATE_ACTIVE];
	      }
	    }
	  else
	    {
	      gdk_gc_set_foreground (text->gc, MARK_CURRENT_FORE (text, &mark));
	      fg_color = MARK_CURRENT_FORE(text, &mark);
	      fg_gc = text->gc;
	    }
	  ...
	  {
#ifdef HAVE_LIBXFT
      	    draw_bg_rect (text, &mark, running_offset, pixel_start_height, pixel_width,
		    LINE_HEIGHT (*lp), FALSE);
	    xft_draw_text (text->text_area, MARK_CURRENT_FONT (text, &mark),
			   fg_gc,
			   fg_color,
			   running_offset,
			   pixel_height,
	    ...
	    gdk_draw_text (text->text_area, MARK_CURRENT_FONT (text, &mark),
          ...
          /* Note that the underline starts drawing at running_offset+1 which
             technically isn't the beginning of the the line, but it "looked
             better" when testing it. */
	  if(MARK_CURRENT_UNDERLINED(text, &mark)) {
	    gdk_draw_line (text->text_area,
			   fg_gc,
			   running_offset+1,
			   pixel_height+1,
			   running_offset+pixel_width-1,
			   pixel_height+1);
	  }
	  if(MARK_CURRENT_DIVIDER(text, &mark))
	  {
	    ...
	    gdk_draw_line (text->text_area,
			   fg_gc,
			   1,
			   pixel_height-(lp->font_ascent/2),
			   width-2,
	  		   pixel_height-(lp->font_ascent/2));
	   }
          if((image=MARK_CURRENT_IMAGE(text, &mark)))
          {
	     GdkBitmap * mask = MARK_CURRENT_MASK(text, &mark);
	     temp_gc = gdk_gc_new(text->text_area);
	     gdk_gc_copy(temp_gc, fg_gc);
#ifdef __MINGW32__
             height=((GdkDrawablePrivate*) image) ->height;
	     pixel_width =((GdkDrawablePrivate*) image) ->width+2;
#else
             height=((GdkWindowPrivate*) image) ->height;
	     pixel_width =((GdkWindowPrivate*) image) ->width+2;
#endif
             height=(pixel_height-height)<pixel_start_height?
                        pixel_height-pixel_start_height:height;
	     
	     if(mask)
	     {
	     	gdk_gc_set_clip_origin(temp_gc, running_offset+1,  pixel_height-height);
		gdk_gc_set_clip_mask(temp_gc, mask);
	     }
           gdk_draw_pixmap(text->text_area,
                           temp_gc,
                           image, 
...
static void
undraw_cursor (ExtGtkText* text, gint absolute)
{
  GtkEditable *editable = (GtkEditable *)text;
...
static gint
drawn_cursor_min (ExtGtkText* text)
{
	  VFont* font;
...
static gint
drawn_cursor_max (ExtGtkText* text)
{
  VFont* font;
...
static void
draw_cursor (ExtGtkText* text, gint absolute)
{
  GtkEditable *editable = (GtkEditable *)text;
...
static void
clear_area (ExtGtkText *text, GdkRectangle *area)
{
  GtkWidget *widget = GTK_WIDGET (text);
...
static void
expose_text (ExtGtkText* text, GdkRectangle *area, gboolean cursor)
{
  GList *cache = text->line_start_cache;
  gint pixels = - text->first_cut_pixels;
...
static void
recompute_geometry (ExtGtkText* text)
{
  ExtGtkPropertyMark mark, start_mark;
  GList *new_lines;
  gint height;
  gint width;
...
#ifdef DEBUG_EXT_GTK_TEXT
static void
gtk_text_show_cache_line (ExtGtkText *text, GList *cache,
			  const char* what, const char* func, gint line)
{
  ...
  for (i = lp->start.index; i < (lp->end.index + lp->wraps); i += 1)
    g_message ("%c", EXT_GTK_TEXT_INDEX (text, i));
  
  g_message (")\n");
}

static void
gtk_text_show_cache (ExtGtkText *text, const char* func, gint line)
{
...
static void
gtk_text_assert_mark (ExtGtkText         *text,
		      ExtGtkPropertyMark *mark,
		      ExtGtkPropertyMark *before,
		      ExtGtkPropertyMark *after,
		      const gchar     *msg,
		      ...
		      gint             line)
{
  ExtGtkPropertyMark correct_mark = find_mark (text, mark->index);
...
static void
gtk_text_assert (ExtGtkText         *text,
		 const gchar     *msg,
		 gint             line)
{
  GList* cache = text->line_start_cache;
  ExtGtkPropertyMark* before_mark = NULL;
  ExtGtkPropertyMark* after_mark = NULL;
...
static void
gtk_text_show_adj (ExtGtkText *text,
		   GtkAdjustment *adj,
		   const char* what,
		   const char* func,
		   gint line)
...
static void
gtk_text_show_props (ExtGtkText *text,
		     const char* msg,
		     int line)
{
