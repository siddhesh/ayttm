...
struct loaded_l10nfile *_nl_make_l10nflist(l10nfile_list, dirlist, dirlist_len,
	...
	special, sponsor, revision, filename, do_allocate)
...
int do_allocate;
{
	...
	abs_filename = (char *)malloc(dirlist_len + strlen(language)
		+ ((mask & TERRITORY) != 0 ? strlen(territory) + 1 : 0)
		+ ((mask & XPG_CODESET) != 0 ? strlen(codeset) + 1 : 0)
		+ ((mask & XPG_NORM_CODESET) != 0
			? strlen(normalized_codeset) + 1 : 0)
		+ (((mask & XPG_MODIFIER) != 0 || (mask & CEN_AUDIENCE) != 0)
			? strlen(modifier) + 1 : 0)
	...
	*cp++ = '/';
	...
	for (; cnt >= 0; --cnt)
		if ((cnt & ~mask) == 0
			...
			while ((dir = __argz_next((char *)dirlist, dirlist_len,
						dir))
				!= NULL)
				retval->successor[entries++]
					= _nl_make_l10nflist(l10nfile_list, dir,
					strlen(dir) + 1, cnt, language,
